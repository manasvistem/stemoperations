...
class Menu_model extends CI_Model{
    ...
    public function get_statetodistrict($stateid){
        $query=$this->db->query("SELECT * FROM district where stateid='$stateid'");
        return $query->result();
    }
    public function get_districttotehsil($districtid){
        $query=$this->db->query("SELECT * FROM tehsil where districtid='$districtid'");
        return $query->result();
    }
    public function get_tehsiltoblock($tehsilid){
        $query=$this->db->query("SELECT * FROM block where tehsilid='$tehsilid'");
        return $query->result();
    }
    public function get_blocktori($blockid){
        $query=$this->db->query("SELECT * FROM ri where blockid='$blockid'");
        return $query->result();
    }
    
    public function get_ritophno($riid){
        $query=$this->db->query("SELECT * FROM phno where riid='$riid'");
        return $query->result();
    }
    
    public function get_phnotovillage($phnoid){
        $query=$this->db->query("SELECT * FROM village where phnoid='$phnoid'");
        return $query->result();
    }
    
    
    public function get_villagetocategory($villageid){
        $query=$this->db->query("SELECT DISTINCT landtype FROM khasrano WHERE vid='$villageid'");
        return $query->result();
    }
    
    public function get_categorytosc($villageid,$category){
        $query=$this->db->query("SELECT DISTINCT sublandtype FROM khasrano WHERE vid='$villageid' and landtype='$category'");
        return $query->result();
    }
    
    
    public function get_landtypetosc($category){
        $query=$this->db->query("SELECT * FROM subland WHERE category='$category'");
        return $query->result();
    }
    
    public function get_villagetokhasrano($vid,$sc){
        $query=$this->db->query("SELECT * FROM khasrano where vid='$vid' and sublandtype='$sc'");
        return $query->result();
    }
    
    public function get_khasranotogc($khasranoid){
        $query=$this->db->query("SELECT * FROM khasranogc where khasranoid='$khasranoid'");
        return $query->result();
    }
    
    public function get_khasranotogcv($khasranoid,$vid){
        $query=$this->db->query("SELECT * FROM khasranogc where khasranoid='$khasranoid' and vid='$vid'");
        return $query->result();
    }
    
    public function get_basracode($khasranoid,$vid){
        
        $query=$this->db->query("SELECT * FROM khasrano where name='$khasranoid' and vid='$vid'");
        $data =  $query->result();
        if($data){
            $basra = $data[0]->barsrano;
        }else{
            $basra=6;
        }
        $query=$this->db->query("SELECT * FROM khasrano where barsrano='$basra' and name!='$khasranoid' and vid='$vid'");
        return   $query->result();
        
    }
    
    public function get_villagegc($vid){
        $query=$this->db->query("SELECT * FROM villagegc where villageid='$vid'");
        return $query->result();
    }
    
    
    
    public function get_khasranotomapinfo($khasranoid,$vid){
        $query=$this->db->query("SELECT *,state.name sname,district.name dname,tehsil.name tname,ri.name rname,village.name vname,khasrano.name kname,khasrano.area,khasrano.landtype,khasrano.landuse,khasrano.owner ownern,khasrano.remarks FROM khasrano LEFT JOIN village ON village.id=khasrano.vid LEFT JOIN phno ON phno.id=village.phnoid LEFT JOIN ri ON ri.id=phno.riid LEFT JOIN block ON block.id=ri.blockid LEFT JOIN tehsil ON tehsil.districtid=block.tehsilid LEFT JOIN district ON district.id=tehsil.districtid LEFT JOIN state ON state.id=district.stateid WHERE khasrano.name='$khasranoid' and khasrano.vid='$vid'");
        return $query->result();
    }
    
    public function state_add($state){
        $query=$this->db->query("INSERT INTO state(name) VALUES ('$state')");
    }
    
    public function district_add($stateid,$district){
        $query=$this->db->query("INSERT INTO district(name,stateid) VALUES ('$district','$stateid')");
    }
    
    public function tehsil_add($districtid,$tehsil){
        $query=$this->db->query("INSERT INTO tehsil(districtid, name) VALUES ('$districtid','$tehsil')");
    }
    
    
    public function block_add($tehsilid,$block){
        $query=$this->db->query("INSERT INTO block(tehsilid, name) VALUES ('$tehsilid','$block')");
    }
    
    public function ri_add($blockid,$ri){
        $query=$this->db->query("INSERT INTO ri(blockid, name) VALUES ('$blockid','$ri')");
    }
    
    public function phno_add($riid,$phno){
        $query=$this->db->query("INSERT INTO phno(riid, name) VALUES ('$riid','$phno')");
    }
    
    
    public function village_add($riid,$village){
        $query=$this->db->query("INSERT INTO village(riid, name) VALUES ('$riid','$village')");
    }
    
    
    
    public function get_knll($knno){
        $query=$this->db->query("SELECT * FROM location where khasrano='$knno'");
        return $query->result();
    }
    public function get_knllog($knno){
        $query=$this->db->query("SELECT * FROM lcod where knno='$knno'");
        return $query->result();
    }
