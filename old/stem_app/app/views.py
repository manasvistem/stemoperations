...
def set_active_user(request,id):
    models.users.objects.filter(user_id=id).update(status="active")
    return redirect("/manage-employees")
    
    
def set_inactive_user(request,id):
    models.users.objects.filter(user_id=id).update(status="inactive")
    return redirect("/manage-employees")
...
def homepage(request):
    ...
    if request.user.is_superuser:
        ...
        if request.method == "POST":
            ...
            if user.type_id == 3 or user.user_id==54:
                data = models.tblcallevents.objects.raw(
                    'SELECT  company_master.*,partner_master.name as partner_type,init_call.*,t1.*,user_details.name,status.name as current_status from tblcallevents t1 join init_call on t1.cid_id=init_call.id  join company_master on init_call.cmpid_id=company_master.id join company_contact_master on company_contact_master.company_id=company_master.id join partner_master on company_master.partnerType_id=partner_master.id join user_details on t1.assignedto_id=user_details.user_id JOIN status on t1.status_id=status.id WHERE t1.assignedto_id =' + str(
                        request.user.id) + ' and t1.appointmentdatetime between "' + from_date + '" and "' + to_date + '"  and nextCFID=0 and t1.id=(SELECT max(id) from tblcallevents t2 WHERE t2.cid_id=t1.cid_id) GROUP by company_master.id  order by t1.appointmentdatetime ASC')
            else:
            ...
            for dt in data:
                if dt.actiontype_id == 1:
                    ...
                    meets.append(dt)
            if user.type_id == 2 and user.user_id !=54:
                # sql = "select * from tblcallevents cr_event join tblcallevents next_event on cr_event.nextCFID=next_event.id "
                opentoreachoutquery = " WHERE cr_event.assignedto_id in %s " % (tuple(
                    usrs),) + " and ( (cr_event.status_id='8' and next_event.status_id='2')) and (cr_event.appointmentdatetime between '" + from_date + "' and '" + to_date + " 23:59:59')"
                opentoopen_rpemquery = " WHERE cr_event.assignedto_id in %s " % (tuple(
                ...
                others = models.tblcallevents.objects.raw(
                    sql + others)
            elif user.type_id == 3 or user.user_id==54:
                # sql = "select * from tblcallevents cr_event join tblcallevents next_event on cr_event.nextCFID=next_event.id "
                opentoreachoutquery = " WHERE cr_event.assignedto_id=" + str(
...
def new_lead(request):
    ...
    states = models.state.objects.filter().order_by('state')
...
def add_new_lead(request):
    ...
    top_spender=request.POST['top_spender']
    action = request.POST['action']
    purpose = request.POST['purpose']
    next_action = request.POST['next_action']
    next_action_date = action_date + ' ' + action_hour + ':' + action_min + ':00'
    form3 = models.tblcalls(cmpid_id=compname, draft=remark, creator=request.user, proposal='no', topspender=top_spender)
    form3.save()
...
def company_details(request, company_id):
    ...
    last_events = models.tblcallevents.objects.filter(cid=company_details).order_by('-id')[:1].get()
    ...
    date = datetime.datetime.now()
    # return HttpResponse(str(last_events))
    return render(request, "company_details.html",
                  {"user":user,"users":users,"conversions": conversions,"last_event":last_events, "open": open, "open_rpem": open_rpem, "reachout": reachout,
...
def all_companies(request):
    ...
    if request.method == "POST":
        ...
        if request.POST['datetype'] == '2':
            ...
            if request.POST['from'] != '' and request.POST['to'] != '':
                ...
                if request.POST['user'] != '':
                    ...
                    # return HttpResponse('SELECT company_contact_master.contactperson,company_contact_master.emailid,company_contact_master.designation,company_contact_master.phoneno , company_master.*,company_master.id as company_id_id,partner_master.name as partner_type,init_call.*,t1.*,user_details.name,status.name as current_status,(select COUNT(*) from tblcallevents WHERE tblcallevents.cid_id=init_call.id and tblcallevents.actiontype_id=1 and tblcallevents.actontaken="yes") as calls,(select COUNT(*) from tblcallevents WHERE tblcallevents.cid_id=init_call.id and tblcallevents.actiontype_id=2 and tblcallevents.actontaken="yes") as emails,(select COUNT(*) from tblcallevents WHERE tblcallevents.cid_id=init_call.id and tblcallevents.actiontype_id=3 and tblcallevents.actontaken="yes") as meets from tblcallevents t1 join init_call on t1.cid_id=init_call.id  join company_master on init_call.cmpid_id=company_master.id join company_contact_master on company_contact_master.company_id=company_master.id join partner_master on company_master.partnerType_id=partner_master.id join user_details on t1.assignedto_id=user_details.user_id JOIN status on t1.status_id=status.id WHERE t1.assignedto_id ='+request.POST["user"]+'  and t1.id=(SELECT max(id) from tblcallevents t2 WHERE t2.cid_id=t1.cid_id) GROUP by company_master.id  order by init_call.id desc')
                else:
            ...
            ddataquery='SELECT cr_event.*,cr_action.name as current_action_type,user_details.name,last_event.appointmentdatetime as last_task_date,last_action.name as last_task_activity,last_event.remarks as last_remark,lpurpose.name as last_purpose,npurpose.name as next_purpose,lstatus.name as last_status,last_event.actontaken as last_action_taken,next_event.appointmentdatetime as next_task_date,nx_action.name as next_task_activity,next_event.remarks as next_remarks,company_master.*,partner_master.name as partner_type,init_call.*,company_contact_master.*,(select status.id from status join tblcallevents g1 on g1.status_id=status.id where g1.id=(select max(id) from tblcallevents h1 where h1.cid_id=cr_event.cid_id ) ) as status_id,(select status.name from status join tblcallevents g1 on g1.status_id=status.id where g1.id=(select max(id) from tblcallevents h1 where h1.cid_id=cr_event.cid_id ) ) as current_status from tblcallevents cr_event left join tblcallevents last_event on cr_event.lastCFID=last_event.id left join action cr_action on cr_action.id=cr_event.actiontype_id left join tblcallevents next_event on cr_event.nextCFID=next_event.id left join status cr_status on cr_status.id=next_event.status_id  left join init_call on init_call.id=cr_event.cid_id left join company_master on company_master.id=init_call.cmpid_id left join action last_action on last_action.id=last_event.actiontype_id left join purpose lpurpose on lpurpose.id=last_event.purpose_id left join purpose npurpose on npurpose.id=next_event.purpose_id left join status lstatus on lstatus.id=cr_event.status_id left join action nx_action on nx_action.id=next_event.actiontype_id left join company_contact_master on company_master.id=company_contact_master.company_id  and company_contact_master.type="primary" left join partner_master on partner_master.id=company_master.partnerType_id  left join user_details on cr_event.assignedto_id=user_details.user_id'
            dataquery='SELECT cr_event.*,cr_status.name as current_status,cr_action.name as current_action_type,user_details.name,last_event.appointmentdatetime as last_task_date,last_action.name as last_task_activity,last_event.remarks as last_remark,lpurpose.name as last_purpose,npurpose.name as next_purpose,lstatus.name as last_status,last_event.actontaken as last_action_taken,next_event.appointmentdatetime as next_task_date,nx_action.name as next_task_activity,next_event.remarks as next_remarks,company_master.*,partner_master.name as partner_type,init_call.*,company_contact_master.*,(select status.id from status join tblcallevents g1 on g1.status_id=status.id where g1.id=(select max(id) from tblcallevents h1 where h1.cid_id=cr_event.cid_id ) ) as current_status_id,(select status.name from status join tblcallevents g1 on g1.status_id=status.id where g1.id=(select max(id) from tblcallevents h1 where h1.cid_id=cr_event.cid_id ) ) as current_status_status from tblcallevents cr_event left join tblcallevents last_event on cr_event.lastCFID=last_event.id left join action cr_action on cr_action.id=cr_event.actiontype_id  left join tblcallevents next_event on cr_event.nextCFID=next_event.id left join status cr_status on cr_status.id=next_event.status_id  left join init_call on init_call.id=cr_event.cid_id left join company_master on company_master.id=init_call.cmpid_id left join action last_action on last_action.id=last_event.actiontype_id left join purpose lpurpose on lpurpose.id=last_event.purpose_id left join purpose npurpose on npurpose.id=next_event.purpose_id left join status lstatus on lstatus.id=cr_event.status_id left join action nx_action on nx_action.id=next_event.actiontype_id left join company_contact_master on company_master.id=company_contact_master.company_id  and company_contact_master.type="primary" left join partner_master on partner_master.id=company_master.partnerType_id join user_details on cr_event.user_id=user_details.user_id'
            if user.type_id == 1:
                users = models.users.objects.filter()
            elif user.type_id == 2:
                users = models.users.objects.filter(Q(user=request.user) | Q(admin=request.user)).filter(
            ...
            if request.POST['from'] != '' and request.POST['to'] != '':
                if request.POST['user'] != '':
                    data = models.tblcalls.objects.raw(dataquery+
                        ' WHERE cr_event.assignedto_id  =' +
                        request.POST[
                            "user"] + ' and  cr_event.appointmentdatetime between "' + from_date + '" and "' + to_date + '"  GROUP by cr_event.id  order by cr_event.id desc'
                    )
                    ...
                    if user.type_id == 3:
                        data = models.tblcalls.objects.raw(dataquery+
                            ' WHERE cr_event.user_id =' + str(
                                request.user.id) + '  and cr_event.appointmentdatetime between "' + from_date + '" and "' + to_date + '"  GROUP by cr_event.id   order by cr_event.id desc')
                        ddata = models.tblcalls.objects.raw(
                            ddataquery+' WHERE cr_event.user_id =' + str(
                                request.user.id) + ' and cr_event.appointmentdatetime between "' + from_date + '" and "' + to_date + '"   GROUP by cr_event.id   order by cr_event.id desc')
                    else:
                        ddata = models.tblcalls.objects.raw(
                            ddataquery+' WHERE cr_event.assignedto_id in %s' % (
                                tuple(
                                    us),) + '  and cr_event.appointmentdatetime between "' + from_date + '" and "' + to_date + '"   GROUP by cr_event.cid_id  order by cr_event.id desc')
                        data = models.tblcalls.objects.raw(dataquery+
                            ' WHERE cr_event.user_id in %s' % (
                                tuple(
                                    us),) + '  and cr_event.appointmentdatetime between "' + from_date + '" and "' + to_date + '"   GROUP by cr_event.id  order by cr_event.id desc')

            else:
                if request.POST['user'] != '':
                    ddata = models.tblcalls.objects.raw(
                        ddataquery+' WHERE cr_event.user_id =' +
                        request.POST[
                            "user"] + ' and cr_event.assignedto_id=(SELECT t3.assignedto_id from tblcallevents t3 where t3.id=(select max(id) from tblcallevents t2 WHERE t2.cid_id=cr_event.cid_id))  GROUP by cr_event.id  order by cr_event.id desc')
                    data = models.tblcalls.objects.raw(dataquery+
                        ' WHERE cr_event.user_id =' +
                        request.POST[
                            "user"] + ' and cr_event.user_id=(SELECT t3.user_id from tblcallevents t3 where t3.id=(select max(id) from tblcallevents t2 WHERE t2.cid_id=cr_event.cid_id))  GROUP by cr_event.id  order by cr_event.id desc')
                    # return HttpResponse('select con.*,company_master.*,company_master.id as cmpny_id,user_details.name,(select status.name from tblcallevents join status on tblcallevents.status_id=status.id WHERE tblcallevents.cid_id=init_call.id order by tblcallevents.id DESC LIMIT 1) as current_status,(select COUNT(*) from tblcallevents WHERE tblcallevents.cid_id=init_call.id) as totaltask,(select COUNT(*) from tblcallevents WHERE tblcallevents.cid_id=init_call.id and tblcallevents.actiontype_id=1 and tblcallevents.actontaken="yes") as calls,(select COUNT(*) from tblcallevents WHERE tblcallevents.cid_id=init_call.id and tblcallevents.actiontype_id=2 and tblcallevents.actontaken="yes") as emails,(select COUNT(*) from tblcallevents WHERE tblcallevents.cid_id=init_call.id and tblcallevents.actiontype_id=3 and tblcallevents.actontaken="yes") as meets,(select status.id from tblcallevents join status on tblcallevents.status_id=status.id WHERE tblcallevents.cid_id=init_call.id order by tblcallevents.id DESC LIMIT 1) as status_id,(select remarks from tblcallevents WHERE tblcallevents.cid_id=init_call.id order by tblcallevents.id DESC LIMIT 1) as last_remark from company_contact_master con join company_master on con.company_id=company_master.id JOIN init_call on init_call.cmpid_id=company_master.id join user_details on init_call.creator_id=user_details.user_id WHERE init_call.creator_id =' +request.POST["user"] + '   order by company_master.id DESC')
                else:
                    if user.type_id == 3:
                        ddata = models.tblcalls.objects.raw(
                            ddataquery+' WHERE cr_event.user_id =' + str(
                                request.user.id) + ' and cr_event.assignedto_id=(SELECT t3.assignedto_id from tblcallevents t3 where t3.id=(select max(id) from tblcallevents t2 WHERE t2.cid_id=cr_event.cid_id))   GROUP by cr_event.id  order by cr_event.id desc')
                        data = models.tblcalls.objects.raw(dataquery+
                            ' WHERE cr_event.user_id =' + str(
                                request.user.id) + ' and cr_event.assignedto_id=(SELECT t3.assignedto_id from tblcallevents t3 where t3.id=(select max(id) from tblcallevents t2 WHERE t2.cid_id=cr_event.cid_id))  GROUP by cr_event.id  order by cr_event.id desc')
                        ...
                        ddata = models.tblcalls.objects.raw(
                            ddataquery+' WHERE cr_event.assignedto_id in %s' % (
                                tuple(
                                    us),) + ' and cr_event.assignedto_id=(SELECT t3.assignedto_id from tblcallevents t3 where t3.id=(select max(id) from tblcallevents t2 WHERE t2.cid_id=cr_event.cid_id))  GROUP by cr_event.id  order by cr_event.id desc')
                        data = models.tblcalls.objects.raw(dataquery+
            ...
            if request.POST['status'] != '':
                for dt in data:
                    if request.POST['currentstatus']!='':    
                        if dt.status_id == int(request.POST['status']) and dt.current_status_id==int(request.POST['currentstatus']):
                            srdata.append(dt)
                    else:
                        if dt.status_id == int(request.POST['status']):
                            srdata.append(dt)
                ...
                data = srdata
            elif request.POST['status']=='' and request.POST['currentstatus']!='':
                for dt in data:
                    if dt.current_status_id==int(request.POST['currentstatus']):
                        srdata.append(dt)
        ...
        if user.type_id == 3:
            data = models.tblcalls.objects.raw(
                'SELECT company_contact_master.contactperson,company_contact_master.emailid,company_contact_master.designation,company_contact_master.phoneno , company_master.*,company_master.id as company_id_id,partner_master.name as partner_type,init_call.*,t1.*,user_details.name,status.name as current_status,action.name as current_action,(select COUNT(*) from tblcallevents WHERE tblcallevents.cid_id=init_call.id ) as totalLogs,(select COUNT(*) from tblcallevents WHERE tblcallevents.cid_id=init_call.id and tblcallevents.actiontype_id=1 and tblcallevents.actontaken="yes") as calls,(select COUNT(*) from tblcallevents WHERE tblcallevents.cid_id=init_call.id and tblcallevents.actiontype_id=2 and tblcallevents.actontaken="yes") as emails,(select COUNT(*) from tblcallevents WHERE tblcallevents.cid_id=init_call.id and tblcallevents.actiontype_id=3 and tblcallevents.actontaken="yes") as meets from tblcallevents t1 join init_call on t1.cid_id=init_call.id  join company_master on init_call.cmpid_id=company_master.id join company_contact_master on company_contact_master.company_id=company_master.id  and company_contact_master.type="primary" join partner_master on company_master.partnerType_id=partner_master.id join user_details on t1.assignedto_id=user_details.user_id JOIN status on t1.status_id=status.id join action on action.id=t1.actiontype_id WHERE t1.assignedto_id =' + str(
                    request.user.id) + '  and t1.id=(SELECT max(id) from tblcallevents t2 WHERE t2.cid_id=t1.cid_id) GROUP by company_master.id  order by init_call.id desc')
...
def unique_companies(request):
    user = models.users.objects.get(user=request.user)
    status = models.status.objects.filter()
    from_date = request.GET['from'] + ' 00:00:00'
    to_date = request.GET['to'] + ' 23:59:59'
    
    if request.user.is_authenticated:
        data = models.tblcalls.objects.raw(
                        'SELECT company_contact_master.contactperson,company_contact_master.emailid,company_contact_master.designation,company_contact_master.phoneno , company_master.*,company_master.id as company_id_id,partner_master.name as partner_type,init_call.*,t1.*,user_details.name,status.name as current_status,action.name as current_action,(select COUNT(*) from tblcallevents WHERE tblcallevents.cid_id=init_call.id ) as totalLogs,(select COUNT(*) from tblcallevents WHERE tblcallevents.cid_id=init_call.id and tblcallevents.actiontype_id=1 and tblcallevents.actontaken="yes") as calls,(select COUNT(*) from tblcallevents WHERE tblcallevents.cid_id=init_call.id and tblcallevents.actiontype_id=2 and tblcallevents.actontaken="yes") as emails,(select COUNT(*) from tblcallevents WHERE tblcallevents.cid_id=init_call.id and tblcallevents.actiontype_id=3 and tblcallevents.actontaken="yes") as meets from tblcallevents t1 join init_call on t1.cid_id=init_call.id  join company_master on init_call.cmpid_id=company_master.id join company_contact_master on company_contact_master.company_id=company_master.id join partner_master on company_master.partnerType_id=partner_master.id join user_details on t1.assignedto_id=user_details.user_id JOIN status on t1.status_id=status.id join action on action.id=t1.actiontype_id WHERE t1.assignedto_id =' +
                        request.GET[
                            "user"] + ' and t1.appointmentdatetime between "' + from_date + '" and "' + to_date + '"    GROUP by t1.cid_id  order by init_call.id desc'
                    )
        

        return render(request, "unique_companies.html",{"data": data})
...
def synopsis(request):
    if request.method != "POST":
        ...
        dataaa = models.tblcallevents.objects.filter(appointmentdatetime__range=(from_date, to_date)).filter(
            Q(assignedto__in=usrs)).filter(Q(actontaken='yes') | Q(actontaken='no')).annotate(dcount=Count('cid_id'))
        ...
        # data.append(dt)
        if user.type_id == 3:
            query = "SELECT  a.*, (SELECT Count(id) FROM tblcallevents t1 WHERE (t1.actontaken='no' or t1.actontaken='yes' ) and t1.assignedto_id = a.user_id  and t1.appointmentdatetime between '" + from_date + "' and '" + to_date + "') as totaltaks  ,(SELECT Count(id) FROM tblcallevents t1 WHERE (t1.actontaken='no' or t1.actontaken='yes' ) and t1.actiontype_id='1' and t1.assignedto_id = a.user_id  and t1.appointmentdatetime between '" + from_date + "' and '" + to_date + "') as totalcalls, (SELECT Count(id) FROM tblcallevents t1 WHERE (t1.actontaken='no' or t1.actontaken='yes' ) and t1.actiontype_id='2' and t1.assignedto_id = a.user_id  and t1.appointmentdatetime between '" + from_date + "' and '" + to_date + "') as totalemails,(SELECT Count(id) FROM tblcallevents t1 WHERE (t1.actontaken='no' or t1.actontaken='yes' ) and t1.actiontype_id='3' and t1.assignedto_id = a.user_id  and t1.appointmentdatetime between '" + from_date + "' and '" + to_date + "') as totalmeets      , (SELECT Count(*) FROM tblcallevents c1 WHERE c1.assignedto_id = a.user_id and c1.actontaken='yes' and updation_data_type='updated' and c1.appointmentdatetime between '" + from_date + "' and '" + to_date + "' ) as completed, (SELECT Count(*) FROM tblcallevents c12 WHERE c12.assignedto_id = a.user_id and c12.actontaken='no' and c12.nextCFID!='0' and updation_data_type='updated' and c12.appointmentdatetime between '" + from_date + "' and '" + to_date + "') as completed_with_no,(SELECT Count(*) FROM tblcallevents c2 WHERE c2.id=(select MAX(cc3.id) from tblcallevents cc3 where cc3.cid_id=c2.cid_id ) and c2.assignedto_id = a.user_id and c2.actontaken='no' and c2.nextCFID='0' and c2.appointmentdatetime between '" + from_date + "' and '" + to_date + "') as pending, (SELECT Count(I1.status_id) FROM tblcallevents I1 WHERE I1.assignedto_id = a.user_id and I1.status_id=1 and I1.status_id=1 and I1.nextCFID='0'  and I1.appointmentdatetime between '" + from_date + "' and '" + to_date + "') as open      ,(SELECT Count(I1.status_id) FROM tblcallevents I1 WHERE I1.assignedto_id = a.user_id and I1.status_id=8 and I1.nextCFID='0' and I1.status_id=8  and I1.appointmentdatetime between '" + from_date + "' and '" + to_date + "') as open_rpem      , (SELECT Count(I2.status_id) FROM tblcallevents I2 WHERE I2.assignedto_id = a.user_id and I2.status_id=2 and I2.status_id=2  and I2.nextCFID='0' and I2.appointmentdatetime between '" + from_date + "' and '" + to_date + "' ) as reachout      , (SELECT Count(I3.status_id) FROM tblcallevents I3 WHERE I3.nextCFID='0' and I3.assignedto_id = a.user_id and I3.status_id=3   and I3.appointmentdatetime between '" + from_date + "' and '" + to_date + "' ) as tantative      , (SELECT Count(I4.status_id) FROM tblcallevents I4 WHERE I4.assignedto_id = a.user_id and I4.status_id=4 and I4.nextCFID='0'  and I4.appointmentdatetime between '" + from_date + "' and '" + to_date + "' ) as will_do_later      , (SELECT Count(I5.status_id) FROM tblcallevents I5 WHERE I5.assignedto_id = a.user_id and I5.status_id=5 and I5.nextCFID='0'  and I5.appointmentdatetime between '" + from_date + "' and '" + to_date + "' ) as not_interested      , (SELECT Count(I6.status_id) FROM tblcallevents I6 WHERE I6.assignedto_id = a.user_id and I6.status_id=6 and I6.nextCFID='0' and I6.appointmentdatetime between '" + from_date + "' and '" + to_date + "' ) as positive,(SELECT Count(I6.status_id) FROM tblcallevents I6 WHERE I6.assignedto_id = a.user_id and I6.nextCFID='0' and I6.status_id=9  and I6.appointmentdatetime between '" + from_date + "' and '" + to_date + "' ) as very_positive      , (SELECT Count(I7.status_id) FROM tblcallevents I7 WHERE I7.assignedto_id = a.user_id and I7.status_id=6 and I7.nextCFID='0' and I7.appointmentdatetime between '" + from_date + "' and '" + to_date + "' ) as closure FROM user_details a where a.status='active' and a.user_id = " + str(
                request.user.id) + ""
            # return HttpResponse(query)

        else:
            query = "SELECT  a.*, (SELECT Count(*) FROM tblcallevents t1 WHERE (t1.actontaken='no' or t1.actontaken='yes' ) and t1.assignedto_id = a.user_id  and t1.appointmentdatetime between '" + from_date + "' and '" + to_date + "') as totaltaks      ,(SELECT Count(id) FROM tblcallevents t1 WHERE (t1.actontaken='no' or t1.actontaken='yes' ) and t1.actiontype_id='1' and t1.assignedto_id = a.user_id  and t1.appointmentdatetime between '" + from_date + "' and '" + to_date + "') as totalcalls, (SELECT Count(id) FROM tblcallevents t1 WHERE (t1.actontaken='no' or t1.actontaken='yes' ) and t1.actiontype_id='2' and t1.assignedto_id = a.user_id  and t1.appointmentdatetime between '" + from_date + "' and '" + to_date + "') as totalemails,(SELECT Count(id) FROM tblcallevents t1 WHERE (t1.actontaken='no' or t1.actontaken='yes' ) and t1.actiontype_id='3' and t1.assignedto_id = a.user_id  and t1.appointmentdatetime between '" + from_date + "' and '" + to_date + "') as totalmeets      , (SELECT Count(*) FROM tblcallevents c1 WHERE c1.assignedto_id = a.user_id and c1.actontaken='yes' and updation_data_type='updated' and c1.appointmentdatetime between '" + from_date + "' and '" + to_date + "' ) as completed, (SELECT Count(*) FROM tblcallevents c12 WHERE c12.assignedto_id = a.user_id and c12.actontaken='no' and c12.nextCFID!='0' and updation_data_type='updated' and c12.appointmentdatetime between '" + from_date + "' and '" + to_date + "') as completed_with_no,(SELECT Count(*) FROM tblcallevents c2 WHERE c2.id=(select MAX(cc3.id) from tblcallevents cc3 where cc3.cid_id=c2.cid_id ) and c2.assignedto_id = a.user_id and c2.actontaken='no' and c2.nextCFID='0' and c2.appointmentdatetime between '" + from_date + "' and '" + to_date + "') as pending, (SELECT Count(I1.status_id) FROM tblcallevents I1 WHERE I1.assignedto_id = a.user_id and I1.status_id=1 and I1.nextCFID='0' and I1.status_id=1  and I1.appointmentdatetime between '" + from_date + "' and '" + to_date + "') as open      ,(SELECT Count(I1.status_id) FROM tblcallevents I1 WHERE I1.assignedto_id = a.user_id and I1.status_id=8 and I1.status_id=8 and I1.nextCFID='0'  and I1.appointmentdatetime between '" + from_date + "' and '" + to_date + "') as open_rpem      , (SELECT Count(I2.status_id) FROM tblcallevents I2 WHERE I2.assignedto_id = a.user_id and I2.status_id=2 and I2.status_id=2 and I2.nextCFID='0'  and I2.appointmentdatetime between '" + from_date + "' and '" + to_date + "' ) as reachout      , (SELECT Count(I3.status_id) FROM tblcallevents I3 WHERE I3.assignedto_id = a.user_id and I3.status_id=3 and I3.nextCFID='0'   and I3.appointmentdatetime between '" + from_date + "' and '" + to_date + "' ) as tantative      , (SELECT Count(I4.status_id) FROM tblcallevents I4 WHERE I4.assignedto_id = a.user_id and I4.status_id=4 and I4.nextCFID='0'  and I4.appointmentdatetime between '" + from_date + "' and '" + to_date + "' ) as will_do_later      , (SELECT Count(I5.status_id) FROM tblcallevents I5 WHERE I5.assignedto_id = a.user_id and I5.status_id=5 and I5.nextCFID='0' and I5.appointmentdatetime between '" + from_date + "' and '" + to_date + "' ) as not_interested      , (SELECT Count(I6.status_id) FROM tblcallevents I6 WHERE I6.assignedto_id = a.user_id and I6.status_id=6 and I6.nextCFID='0' and I6.appointmentdatetime between '" + from_date + "' and '" + to_date + "' ) as positive,(SELECT Count(I6.status_id) FROM tblcallevents I6 WHERE I6.assignedto_id = a.user_id and I6.nextCFID='0' and I6.status_id=9  and I6.appointmentdatetime between '" + from_date + "' and '" + to_date + "' ) as very_positive      , (SELECT Count(I7.status_id) FROM tblcallevents I7 WHERE I7.assignedto_id = a.user_id and I7.status_id=6 and I7.nextCFID='0' and I7.appointmentdatetime between '" + from_date + "' and '" + to_date + "' ) as closure FROM user_details a where a.status='active' and a.user_id in %s" % (
                tuple(usrs),)
        ...
        return render(request, "synopsis.html",
                      {"open": open, "open_rpem": open_rpem, "reachout": reachout, "tentative": tentative,
                       ...
                       "emails": len(emails),
                       "calls": len(calls), "meets": len(meets), "from": date, "to": date,"fromDate":"future"})
        ...
        if user.type_id == 3:
            query = "SELECT  a.*, (SELECT Count(*) FROM tblcallevents t1 WHERE (t1.actontaken='no' or t1.actontaken='yes' ) and t1.assignedto_id = a.user_id  and t1.appointmentdatetime between '" + from_data + "' and '" + to_data + "') as totaltaks      ,(SELECT Count(id) FROM tblcallevents t1 WHERE (t1.actontaken='no' or t1.actontaken='yes' ) and t1.actiontype_id='1' and t1.assignedto_id = a.user_id  and t1.appointmentdatetime between '" + from_data + "' and '" + to_data + "') as totalcalls, (SELECT Count(id) FROM tblcallevents t1 WHERE (t1.actontaken='no' or t1.actontaken='yes' ) and t1.actiontype_id='2' and t1.assignedto_id = a.user_id  and t1.appointmentdatetime between '" + from_data + "' and '" + to_data + "') as totalemails,(SELECT Count(id) FROM tblcallevents t1 WHERE (t1.actontaken='no' or t1.actontaken='yes' ) and t1.actiontype_id='3' and t1.assignedto_id = a.user_id  and t1.appointmentdatetime between '" + from_data + "' and '" + to_data + "') as totalmeets      , (SELECT Count(*) FROM tblcallevents c1 WHERE c1.assignedto_id = a.user_id and c1.actontaken='yes' and updation_data_type='updated' and c1.appointmentdatetime between '" + from_data + "' and '" + to_data + "' ) as completed, (SELECT Count(*) FROM tblcallevents c12 WHERE c12.assignedto_id = a.user_id and c12.actontaken='no' and updation_data_type='updated' and c12.nextCFID!='0' and c12.appointmentdatetime between '" + from_data + "' and '" + to_data + "') as completed_with_no,(SELECT Count(*) FROM tblcallevents c2 WHERE (c2.id=(select MAX(cc3.id) from tblcallevents cc3 where cc3.cid_id=c2.cid_id ) or updation_data_type!='updated') and c2.assignedto_id = a.user_id  and c2.appointmentdatetime between '" + from_data + "' and '" + to_data + "') as pending, (SELECT Count(I1.status_id) FROM tblcallevents I1 WHERE I1.assignedto_id = a.user_id and I1.status_id=1 and I1.status_id=1 and I1.nextCFID='0'  and I1.appointmentdatetime between '" + from_data + "' and '" + to_data + "') as open      ,(SELECT Count(I1.status_id) FROM tblcallevents I1 WHERE I1.assignedto_id = a.user_id and I1.status_id=8 and I1.nextCFID='0' and I1.status_id=8  and I1.appointmentdatetime between '" + from_data + "' and '" + to_data + "') as open_rpem      , (SELECT Count(I2.status_id) FROM tblcallevents I2 WHERE I2.assignedto_id = a.user_id and I2.status_id=2 and I2.status_id=2  and I2.nextCFID='0' and I2.appointmentdatetime between '" + from_data + "' and '" + to_data + "' ) as reachout      , (SELECT Count(I3.status_id) FROM tblcallevents I3 WHERE I3.nextCFID='0' and I3.assignedto_id = a.user_id and I3.status_id=3   and I3.appointmentdatetime between '" + from_data + "' and '" + to_data + "' ) as tantative      , (SELECT Count(I4.status_id) FROM tblcallevents I4 WHERE I4.assignedto_id = a.user_id and I4.status_id=4 and I4.nextCFID='0'  and I4.appointmentdatetime between '" + from_data + "' and '" + to_data + "' ) as will_do_later      , (SELECT Count(I5.status_id) FROM tblcallevents I5 WHERE I5.assignedto_id = a.user_id and I5.status_id=5 and I5.nextCFID='0'  and I5.appointmentdatetime between '" + from_data + "' and '" + to_data + "' ) as not_interested      , (SELECT Count(I6.status_id) FROM tblcallevents I6 WHERE I6.assignedto_id = a.user_id and I6.status_id=6 and I6.nextCFID='0' and I6.appointmentdatetime between '" + from_data + "' and '" + to_data + "' ) as positive,(SELECT Count(I6.status_id) FROM tblcallevents I6 WHERE I6.assignedto_id = a.user_id and I6.nextCFID='0' and I6.status_id=9  and I6.appointmentdatetime between '" + from_data + "' and '" + to_data + "' ) as very_positive      , (SELECT Count(I7.status_id) FROM tblcallevents I7 WHERE I7.assignedto_id = a.user_id and I7.status_id=6 and I7.nextCFID='0' and I7.appointmentdatetime between '" + from_data + "' and '" + to_data + "' ) as closure FROM user_details a where a.status='active' and a.user_id = (" + str(
                request.user.id) + ")"
            # return HttpResponse(query)

        else:
            query = "SELECT  a.*, (SELECT Count(*) FROM tblcallevents t1 WHERE (t1.actontaken='no' or t1.actontaken='yes' ) and t1.assignedto_id = a.user_id  and t1.appointmentdatetime between '" + from_data + "' and '" + to_data + "') as totaltaks      ,(SELECT Count(id) FROM tblcallevents t1 WHERE (t1.actontaken='no' or t1.actontaken='yes' ) and t1.actiontype_id='1' and t1.assignedto_id = a.user_id  and t1.appointmentdatetime between '" + from_data + "' and '" + to_data + "') as totalcalls, (SELECT Count(id) FROM tblcallevents t1 WHERE (t1.actontaken='no' or t1.actontaken='yes' ) and t1.actiontype_id='2' and t1.assignedto_id = a.user_id  and t1.appointmentdatetime between '" + from_data + "' and '" + to_data + "') as totalemails,(SELECT Count(id) FROM tblcallevents t1 WHERE (t1.actontaken='no' or t1.actontaken='yes' ) and t1.actiontype_id='3' and t1.assignedto_id = a.user_id  and t1.appointmentdatetime between '" + from_data + "' and '" + to_data + "') as totalmeets      , (SELECT Count(*) FROM tblcallevents c1 WHERE c1.assignedto_id = a.user_id and c1.actontaken='yes' and updation_data_type='updated' and c1.appointmentdatetime between '" + from_data + "' and '" + to_data + "' ) as completed, (SELECT Count(*) FROM tblcallevents c12 WHERE c12.assignedto_id = a.user_id and c12.actontaken='no' and updation_data_type='updated' and c12.nextCFID!='0' and c12.appointmentdatetime between '" + from_data + "' and '" + to_data + "') as completed_with_no,(SELECT Count(*) FROM tblcallevents c2 WHERE (c2.id=(select MAX(cc3.id) from tblcallevents cc3 where cc3.cid_id=c2.cid_id ) or updation_data_type!='updated') and c2.assignedto_id = a.user_id  and c2.appointmentdatetime between '" + from_data + "' and '" + to_data + "') as pending, (SELECT Count(I1.status_id) FROM tblcallevents I1 WHERE I1.assignedto_id = a.user_id and I1.status_id=1 and I1.nextCFID='0' and I1.status_id=1  and I1.appointmentdatetime between '" + from_data + "' and '" + to_data + "') as open      ,(SELECT Count(I1.status_id) FROM tblcallevents I1 WHERE I1.assignedto_id = a.user_id and I1.status_id=8 and I1.status_id=8 and I1.nextCFID='0'  and I1.appointmentdatetime between '" + from_data + "' and '" + to_data + "') as open_rpem      , (SELECT Count(I2.status_id) FROM tblcallevents I2 WHERE I2.assignedto_id = a.user_id and I2.status_id=2 and I2.status_id=2 and I2.nextCFID='0'  and I2.appointmentdatetime between '" + from_data + "' and '" + to_data + "' ) as reachout      , (SELECT Count(I3.status_id) FROM tblcallevents I3 WHERE I3.assignedto_id = a.user_id and I3.status_id=3 and I3.nextCFID='0'   and I3.appointmentdatetime between '" + from_data + "' and '" + to_data + "' ) as tantative      , (SELECT Count(I4.status_id) FROM tblcallevents I4 WHERE I4.assignedto_id = a.user_id and I4.status_id=4 and I4.nextCFID='0'  and I4.appointmentdatetime between '" + from_data + "' and '" + to_data + "' ) as will_do_later      , (SELECT Count(I5.status_id) FROM tblcallevents I5 WHERE I5.assignedto_id = a.user_id and I5.status_id=5 and I5.nextCFID='0' and I5.appointmentdatetime between '" + from_data + "' and '" + to_data + "' ) as not_interested      , (SELECT Count(I6.status_id) FROM tblcallevents I6 WHERE I6.assignedto_id = a.user_id and I6.status_id=6 and I6.nextCFID='0' and I6.appointmentdatetime between '" + from_data + "' and '" + to_data + "' ) as positive,(SELECT Count(I6.status_id) FROM tblcallevents I6 WHERE I6.assignedto_id = a.user_id and I6.nextCFID='0' and I6.status_id=9  and I6.appointmentdatetime between '" + from_data + "' and '" + to_data + "' ) as very_positive      , (SELECT Count(I7.status_id) FROM tblcallevents I7 WHERE I7.assignedto_id = a.user_id and I7.status_id=6 and I7.nextCFID='0' and I7.appointmentdatetime between '" + from_data + "' and '" + to_data + "' ) as closure FROM user_details a where a.status='active' and a.user_id in %s" %(tuple(usrs),)
            # return HttpResponse(query)

        data = models.users.objects.raw(query)
        from datetime import date
        testdate = date.today()
        checkdate = testdate.strftime('%Y-%m-%d')
        if request.POST["from"] >=checkdate:
            dateType="future"
        else:
            dateType="past"
        return render(request, "synopsis.html",
                      {"open": open, "open_rpem": open_rpem, "reachout": reachout, "tentative": tentative,
                       ...
                       "to": request.POST["to"],"fromDate":dateType})


@login_required(login_url='/')
def employee_data_calls(request,id):
    user = models.users.objects.get(user=request.user)
    if user.type_id == 1:
        users = models.users.objects.filter()
    elif user.type_id == 2:
    ...
    if request.method == "POST":
        ...
        tentativetopositivequery = " WHERE cr_event.assignedto_id =" + str(id) + " and (cr_event.actontaken='no' or cr_event.actontaken='yes' ) and cr_event.actiontype_id='1' (cr_event.appointmentdatetime BETWEEN '" + from_date + "' and '" + to_date + "') group by cr_event.id"
        # else:

        #     tentativetopositivequery = " WHERE cr_event.assignedto_id in %s " % (tuple(
        #         usrs),) + " and  (cr_event.actontaken='no' or cr_event.actontaken='yes' ) and cr_event.actiontype_id='1' and (cr_event.appointmentdatetime BETWEEN '" + from_date + "' and '" + to_date + "') group by cr_event.id"
        ...
        tentativetopositivequery = " WHERE cr_event.assignedto_id = " + str(id) + " and (cr_event.actontaken='no' or cr_event.actontaken='yes' ) and cr_event.actiontype_id='1' and (cr_event.appointmentdatetime BETWEEN '" + from_date + "' and '" + to_date + "') group by cr_event.id"
        # else:
        ...
        return render(request, "conversiondetailsreports.html",
                      {"from":request.GET["from"], "to":request.GET["to"],"con_name":request.GET.get('con_name'),"data": data, "open": open, "open_rpem": open_rpem, "reachout": reachout, "tentative": tentative,
                       "will_do_later": will_do_later, "ni": ni, "very_positive": very_positive, "closure": closure,
                       "all_len": len(data), "open_len": len(open), "open_rpem_len": len(open_rpem),
                       "reachout_len": len(reachout),
                       "tentative_len": len(tentative), "will_do_later_len": len(will_do_later), "ni_len": len(ni),
...
@login_required(login_url='/')
def employee_data_emails(request,id):
    user = models.users.objects.get(user=request.user)
    if user.type_id == 1:
        users = models.users.objects.filter()
    elif user.type_id == 2:
    ...
    if request.method == "POST":
        ...
        tentativetopositivequery = " WHERE user_details.user_id =" + str(
                id) + " and (cr_event.actontaken='no' or cr_event.actontaken='yes' ) and cr_event.actiontype_id='2' and (cr_event.appointmentdatetime BETWEEN '" + from_date + "' and '" + to_date + "') group by cr_event.id"
        # else:

        #     tentativetopositivequery = " WHERE user_details.user_id in %s " % (tuple(
        #         usrs),) + " and (cr_event.actontaken='no' or cr_event.actontaken='yes' ) and cr_event.actiontype_id='2' and (cr_event.appointmentdatetime BETWEEN '" + from_date + "' and '" + to_date + "') group by cr_event.id"
        data = models.tblcallevents.objects.raw(reportssql + tentativetopositivequery)
        open = []
        open_rpem = []
        reachout = []
        ...
        tentativetopositivequery = " WHERE user_details.user_id = " + str(id) + " and (cr_event.actontaken='no' or cr_event.actontaken='yes' ) and cr_event.actiontype_id='2' and (cr_event.appointmentdatetime BETWEEN '" + from_date + "' and '" + to_date + "') group by cr_event.id"
        data = models.tblcallevents.objects.raw(reportssql + tentativetopositivequery)
        open = []
        open_rpem = []
        reachout = []
...
@login_required(login_url='/')
def employee_data_meets(request,id):
    user = models.users.objects.get(user=request.user)
    if user.type_id == 1:
        users = models.users.objects.filter()
    elif user.type_id == 2:
    ...
    if request.method == "POST":
        ...
        tentativetopositivequery = " WHERE user_details.user_id =" + str(id) + " and (cr_event.actontaken='no' or cr_event.actontaken='yes' ) and cr_event.actiontype_id='3' and (cr_event.appointmentdatetime BETWEEN '" + from_date + "' and '" + to_date + "') group by cr_event.id"
        # else:

        #     tentativetopositivequery = " WHERE user_details.user_id in %s " % (tuple(
        #         usrs),) + " and (cr_event.actontaken='no' or cr_event.actontaken='yes' ) and cr_event.actiontype_id='3' and (cr_event.appointmentdatetime BETWEEN '" + from_date + "' and '" + to_date + "') group by cr_event.id"
        data = models.tblcallevents.objects.raw(reportssql + tentativetopositivequery)
        open = []
        open_rpem = []
        reachout = []
...
def add_call_log(request):
    if request.method == "POST":
        ...
        action_hour = request.POST['action_hour']
        action_min = request.POST['action_min']
        meeting_type=request.POST.get('meeting_type',False)
        ...
        if proposal_sent != False:
            models.tblcalls.objects.filter(cmpid_id=cid).update(draft=proposal_description, proposal=proposal_sent,proposal_type=request.POST['proposal_type'],proposal_amt=request.POST['proposal_amt'],noofschools=request.POST['no_of_school'],proposaldate=date.today())
        ...
        if lastCFID:
            ...
            if lastevent.appointmentdatetime>date:
                
                models.tblcallevents.objects.filter(id=lastCFID).update(appointmentdatetime=date)
            ...
            if count > 1:
                models.tblcallevents.objects.filter(id=lastCFID).update(draft=proposal_description,
                                                                        actontaken=action_taken,updation_data_type="updated" )
            else:
                models.tblcallevents.objects.filter(id=lastCFID).update(draft=proposal_description,
                                                                        actontaken=action_taken,updation_data_type="updated")
...
def manage_employees(request):
    ...
    if user.type_id == 1:
        ...
        users = models.users.objects.filter(Q(user=request.user) | Q(admin=request.user))
    else:
        users = models.users.objects.filter(user=request.user)
...
def employee_data(request, id):
    ...
    if request.method == "POST":
        ...
        pending = models.tblcallevents.objects.raw("SELECT * FROM tblcallevents c2 WHERE c2.id=(select MAX(cc3.id) from tblcallevents cc3 where cc3.cid_id=c2.cid_id ) and c2.assignedto_id = "+str(id)+" and c2.actontaken='no' and c2.nextCFID='0' and c2.appointmentdatetime between '" + from_date + "' and '" + to_date + " 23:59:59'")
...
def forward_to_current_date(request, cmpid, last_id):
    ...
    form4 = models.tblcallevents(cid=data.cid, user=request.user, lastCFID=last_id, nextCFID=0,
                                 ...
                                 nextaction=data.nextaction,updation_data_type="forwarded")
    form4.save()
...
def update_company(request, company_id):
    ...
    # state_count = models.state.objects.filter(Q(state=state)).count()
    # if state_count > 0:
    #     state_name = models.state.objects.filter(Q(state=state))[:1].get()
    # else:
    #     state_name = []
    ...
    models.company_master.objects.filter(id=company_id).update(compname=compname, draft=draft, budget=budget,
                                                               address=address,
                                                               country=country, website=website, state=state,
...
@login_required(login_url='/')
def setasalternate(request, company_id, contact_id):
    if request.method == "GET":
        ...
        # fm=models.notification(msg='Contact of '+str(dt.compname)+' has been updated by '+str(request.user.first_name),user=str(users.admin_id),company_id=str(company_id),status='pending')
        # fm.save()
        models.company_contact_master.objects.filter(id=contact_id).update(type='alternate')

        return redirect('/company/' + str(company_id))


@login_required(login_url='/')
def setasprimary(request, company_id, contact_id):
    if request.method == "GET":
        ...
        # fm=models.notification(msg='Contact of '+str(dt.compname)+' has been updated by '+str(request.user.first_name),user=str(users.admin_id),company_id=str(company_id),status='pending')
        # fm.save()
        models.company_contact_master.objects.filter(id=contact_id).update(type='primary')

        return redirect('/company/' + str(company_id))

@login_required(login_url='/')
def assigned_data(request):
    status_field = models.status.objects.filter()
    if request.method == "POST":
        from datetime import datetime
        events = request.POST.getlist('init_id')
        assigned_id = request.POST['assigned_to']
        assigned_notes = request.POST['assigned_notes']
        selected_user=request.POST['selected_user']
        filter_from=request.POST['filter_from']
        filter_to=request.POST['filter_to']
        filter_status=request.POST['filter_status']
        filter_type=request.POST['filter_type']
        if assigned_notes!="":
            remark_notes=assigned_notes
        else:
            remark_notes="Task has been assigned to " + user.first_name + ' ' + user.last_name
        ...
        return redirect('/assigned-data?selected_user='+selected_user+'&type='+filter_type+'&status='+filter_status+'&from='+filter_from+'&to='+filter_to)

    else:
        user = models.users.objects.get(user=request.user)
        if user.type_id == 1:
            users = models.users.objects.filter(type_id=2)
            searching_users = models.users.objects.filter(type_id=2)
        elif user.type_id == 2:
            users = models.users.objects.filter(Q(user=request.user) | Q(admin=request.user) | Q(type_id=2)).filter(
                status='active')
            searching_users = models.users.objects.filter(Q(user=request.user) | Q(admin=request.user)).filter(
                status='active')
        else:
            users = models.users.objects.filter(user=request.user).filter(status='active').filter(type_id=2)
            searching_users = models.users.objects.filter(user=request.user).filter(status='active').filter(type_id=2)
        usrs = []
        ...
        if 'from' in request.GET:
            if request.GET["from"]!='':
                ...
                date_query=" and t1.fwd_date between '"+request.GET['from']+" 00:00:00' and '"+request.GET['to']+" 23:59:59' "
            else:
        ...
        if 'status' in request.GET:
            if request.GET["status"]!="":
                status_query=" and t1.status_id ="+request.GET["status"]+" group by t1.cid_id order by t1.id desc "
            else:
                status_query=" group by t1.cid_id order by t1.id desc "
        else:
            status_query=" group by t1.cid_id order by t1.id desc "
        ...
        if selected_user!='':
            ...
            if user.type_id == 2:
                # return HttpResponse("SELECT t1.*,company_master.*,init_call.*,user_details.name FROM tblcallevents t1 join init_call on t1.cid_id=init_call.id JOIN user_details on init_call.creator_id=user_details.user_id join company_master on company_master.id=init_call.cmpid_id where t1.id=(select MAX(id) from tblcallevents t2 where t2.cid_id=t1.cid_id) and (t1.assignedto_id=" + selected_user + " and t1.assignedto_id!=init_call.creator_id) and (init_call.creator_id="+selected_user+" or t1.assignedto_id=" + selected_user +") "+date_query)
                if request.GET["type"]=="from":
                    data = models.tblcallevents.objects.raw("SELECT t1.*,company_master.*,init_call.*,user_details.name FROM tblcallevents t1 join init_call on t1.cid_id=init_call.id left join tblcallevents t2 on t1.cid_id=t2.cid_id and t1.assignedto_id!=t2.assignedto_id and t2.assignedto_id=(select assignedto_id from tblcallevents WHERE tblcallevents.cid_id=t1.cid_id and t1.assignedto_id!=tblcallevents.assignedto_id order by tblcallevents.id desc limit 1) JOIN user_details on t2.assignedto_id=user_details.user_id join company_master on company_master.id=init_call.cmpid_id  where t1.id=(select MAX(id) from tblcallevents t2 where t2.cid_id=t1.cid_id) and ( t1.assignedto_id!=init_call.creator_id) and (t2.assignedto_id="+selected_user+" ) "+date_query+status_query)
                    # return HttpResponse("SELECT t1.*,company_master.*,init_call.*,user_details.name FROM tblcallevents t1 join init_call on t1.cid_id=init_call.id JOIN user_details on init_call.creator_id=user_details.user_id join company_master on company_master.id=init_call.cmpid_id left join tblcallevents t2 on t1.cid_id=t2.cid_id and t1.assignedto_id!=t2.assignedto_id and t2.assignedto_id=(select assignedto_id from tblcallevents WHERE tblcallevents.cid_id=t1.cid_id and t1.assignedto_id!=tblcallevents.assignedto_id order by tblcallevents.id desc limit 1) where t1.id=(select MAX(id) from tblcallevents t2 where t2.cid_id=t1.cid_id) and ( t1.assignedto_id!=init_call.creator_id) and (t2.assignedto_id="+selected_user+" ) "+date_query+status_query)
                if request.GET["type"]=="to":
                    data = models.tblcallevents.objects.raw("SELECT t1.*,company_master.*,init_call.*,user_details.name FROM tblcallevents t1 join init_call on t1.cid_id=init_call.id left join tblcallevents t2 on t1.cid_id=t2.cid_id and t1.assignedto_id!=t2.assignedto_id and t2.assignedto_id=(select assignedto_id from tblcallevents WHERE tblcallevents.cid_id=t1.cid_id and t1.assignedto_id!=tblcallevents.assignedto_id order by tblcallevents.id desc limit 1) JOIN user_details on t2.assignedto_id=user_details.user_id join company_master on company_master.id=init_call.cmpid_id  where t1.id=(select MAX(id) from tblcallevents t2 where t2.cid_id=t1.cid_id) and ( t1.assignedto_id!=init_call.creator_id) and (t1.assignedto_id=" + selected_user +") "+date_query+status_query)
            elif user.type_id == 1:
                if request.GET["type"]=="from":
                    data = models.tblcallevents.objects.raw("SELECT t1.*,company_master.*,init_call.*,user_details.name FROM tblcallevents t1 join init_call on t1.cid_id=init_call.id left join tblcallevents t2 on t1.cid_id=t2.cid_id and t1.assignedto_id!=t2.assignedto_id and t2.assignedto_id=(select assignedto_id from tblcallevents WHERE tblcallevents.cid_id=t1.cid_id and t1.assignedto_id!=tblcallevents.assignedto_id order by tblcallevents.id desc limit 1) JOIN user_details on t2.assignedto_id=user_details.user_id join company_master on company_master.id=init_call.cmpid_id  where t1.id=(select MAX(id) from tblcallevents t2 where t2.cid_id=t1.cid_id) and (t1.assignedto_id!=init_call.creator_id) and (t2.assignedto_id="+selected_user+") "+date_query+status_query)
                if request.GET["type"]=="to":
            ...
            if user.type_id == 2:
                ...
                #     request.user.id) + " and init_call.creator_id!=" + str(request.user.id) + ") "+date_query+" ORDER BY 'nextCFID'  DESC")
                data = models.tblcallevents.objects.raw("SELECT t1.*,company_master.*,init_call.*,user_details.name FROM tblcallevents t1 join init_call on t1.cid_id=init_call.id left join tblcallevents t2 on t1.cid_id=t2.cid_id and t1.assignedto_id!=t2.assignedto_id and t2.assignedto_id=(select assignedto_id from tblcallevents WHERE tblcallevents.cid_id=t1.cid_id and t1.assignedto_id!=tblcallevents.assignedto_id order by tblcallevents.id desc limit 1) JOIN user_details on t2.assignedto_id=user_details.user_id join company_master on company_master.id=init_call.cmpid_id  where t1.id=(select MAX(id) from tblcallevents t2 where t2.cid_id=t1.cid_id) and (t1.assignedto_id=" + str(request.user.id) + " and init_call.creator_id!=" + str(request.user.id) + ") "+date_query+status_query)
            elif user.type_id == 1:
                data = models.tblcallevents.objects.raw("SELECT t1.*,company_master.*,init_call.*,user_details.name FROM tblcallevents t1 join init_call on t1.cid_id=init_call.id left join tblcallevents t2 on t1.cid_id=t2.cid_id and t1.assignedto_id!=t2.assignedto_id and t2.assignedto_id=(select assignedto_id from tblcallevents WHERE tblcallevents.cid_id=t1.cid_id and t1.assignedto_id!=tblcallevents.assignedto_id order by tblcallevents.id desc limit 1) JOIN user_details on t2.assignedto_id=user_details.user_id join company_master on company_master.id=init_call.cmpid_id  where t1.id=(select MAX(id) from tblcallevents t2 where t2.cid_id=t1.cid_id) and (t1.assignedto_id in %s" % (tuple(usrs),) + " and t1.assignedto_id!=init_call.creator_id) "+date_query+status_query)
        ...
        return render(request, 'assigned_data.html', {"data": data, "count": len(data), "users": users,"searching_users":searching_users,"status_field":status_field,"selected_user":selected_user,"filter_from":filter_from,"filter_to":filter_to,"filter_status":filter_status,"filter_type":filter_type})


@login_required(login_url='/')
def export_assigned_data(request):
    status_field = models.status.objects.filter()
    if request.method == "POST":
        from datetime import datetime
        events = request.POST.getlist('init_id')
        ...
        if selected_user!='':
            ...
            if user.type_id == 2:
                # return HttpResponse("SELECT t1.*,company_master.*,init_call.*,user_details.name FROM tblcallevents t1 join init_call on t1.cid_id=init_call.id JOIN user_details on init_call.creator_id=user_details.user_id join company_master on company_master.id=init_call.cmpid_id where t1.id=(select MAX(id) from tblcallevents t2 where t2.cid_id=t1.cid_id) and (t1.assignedto_id=" + selected_user + " and t1.assignedto_id!=init_call.creator_id) and (init_call.creator_id="+selected_user+" or t1.assignedto_id=" + selected_user +") "+date_query)
                if request.GET["type"]=="from":
                    data = models.tblcallevents.objects.raw("SELECT t1.*,company_master.*,company_contact_master.*, init_call.*,user_details.name FROM tblcallevents t1 join init_call on t1.cid_id=init_call.id left join tblcallevents t2 on t1.cid_id=t2.cid_id and t1.assignedto_id!=t2.assignedto_id and t2.assignedto_id=(select assignedto_id from tblcallevents WHERE tblcallevents.cid_id=t1.cid_id and t1.assignedto_id!=tblcallevents.assignedto_id order by tblcallevents.id desc limit 1) JOIN user_details on t2.assignedto_id=user_details.user_id join company_master on company_master.id=init_call.cmpid_id  where t1.id=(select MAX(id) from tblcallevents t2 where t2.cid_id=t1.cid_id) and ( t1.assignedto_id!=init_call.creator_id) and (t2.assignedto_id="+selected_user+" ) "+date_query+status_query)
                    # return HttpResponse("SELECT t1.*,company_master.*,init_call.*,user_details.name FROM tblcallevents t1 join init_call on t1.cid_id=init_call.id JOIN user_details on init_call.creator_id=user_details.user_id join company_master on company_master.id=init_call.cmpid_id left join tblcallevents t2 on t1.cid_id=t2.cid_id and t1.assignedto_id!=t2.assignedto_id and t2.assignedto_id=(select assignedto_id from tblcallevents WHERE tblcallevents.cid_id=t1.cid_id and t1.assignedto_id!=tblcallevents.assignedto_id order by tblcallevents.id desc limit 1) where t1.id=(select MAX(id) from tblcallevents t2 where t2.cid_id=t1.cid_id) and ( t1.assignedto_id!=init_call.creator_id) and (t2.assignedto_id="+selected_user+" ) "+date_query+status_query)
                if request.GET["type"]=="to":
                    data = models.tblcallevents.objects.raw("SELECT t1.*,company_master.*,init_call.*,user_details.name FROM tblcallevents t1 join init_call on t1.cid_id=init_call.id left join tblcallevents t2 on t1.cid_id=t2.cid_id and t1.assignedto_id!=t2.assignedto_id and t2.assignedto_id=(select assignedto_id from tblcallevents WHERE tblcallevents.cid_id=t1.cid_id and t1.assignedto_id!=tblcallevents.assignedto_id order by tblcallevents.id desc limit 1) JOIN user_details on t2.assignedto_id=user_details.user_id join company_master on company_master.id=init_call.cmpid_id  where t1.id=(select MAX(id) from tblcallevents t2 where t2.cid_id=t1.cid_id) and ( t1.assignedto_id!=init_call.creator_id) and (t1.assignedto_id=" + selected_user +") "+date_query+status_query)
            elif user.type_id == 1:
            ...
            # return HttpResponse("SELECT t1.*,company_master.*,init_call.*,user_details.* FROM tblcallevents t1 join init_call on t1.cid_id=init_call.id JOIN user_details on init_call.creator_id=user_details.user_id join company_master on company_master.id=init_call.cmpid_id where t1.id=(select MAX(id) from tblcallevents t2 where t2.cid_id=t1.cid_id) and (t1.assignedto_id in %s" %(tuple(usrs),) + ") ORDER BY `nextCFID`  DESC")
        return render(request, 'export_assigned_data.html', {"data": data, "count": len(data), "users": users,"searching_users":searching_users,"status_field":status_field,"selected_user":selected_user,"filter_from":filter_from,"filter_to":filter_to,"filter_status":filter_status,"filter_type":filter_type})
...
def my_data(request):
    ...
    if request.method == "POST":
        ...
        if selected_user!='':
            ...
            # return HttpResponse("SELECT t1.*,company_master.*,init_call.*,user_details.* FROM tblcallevents t1 join init_call on t1.cid_id=init_call.id JOIN user_details on init_call.creator_id=user_details.user_id join company_master on company_master.id=init_call.cmpid_id where t1.id=(select MAX(id) from tblcallevents t2 where t2.cid_id=t1.cid_id) and (t1.assignedto_id in %s" %(tuple(usrs),) + ") ORDER BY `nextCFID`  DESC")
        return render(request, 'my_data.html', {"data": data, "count": len(data), "users": users,"searching_users":searching_users,"status_field":status_field,"selected_user":selected_user,"filter_from":filter_from,"filter_to":filter_to,"filter_status":filter_status,"filter_type":filter_type})
...
def task_conversoin_report(request):
    ...
    if request.method == "POST":
        ...
        from_date_d = date.today().strftime('%Y-%m') + "-01"
        to_date_d = date.today().strftime('%Y-%m-%d')
    if request.method=="POST" and request.POST["dateType"]=="all_date":
        if user.type_id == 3:
            query = "SELECT user_details.*,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=1 and t2.status_id=8 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=1 ))*100 as open_to_open_rpem,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=8 and t2.status_id=2 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=8 ))*100 as open_rpem_to_reachout,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=2 and t2.status_id=3 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=2 ))*100 as reachout_to_tentative,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=3 and t2.status_id=6 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=3 ))*100 as tentative_to_verypositive,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=6 and t2.status_id=9 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=6 ))*100 as positive_to_verypositive,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=9 and t2.status_id=7 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=9 ))*100 as verypositive_to_closure,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=8 and t2.status_id=4 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=8 ))*100 as open_to_willdolater,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=8 and t2.status_id=5 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=8 ))*100 as open_to_negative,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=2 and t2.status_id=4 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=2 ))*100 as reachout_to_willdolater,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=2 and t2.status_id=5 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=2 ))*100 as rechout_negative,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=3 and t2.status_id=4 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=3 ))*100 as tentative_willdolater,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=3 and t2.status_id=5 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=3 ))*100 as tentative_negative,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=5 and t2.status_id=2 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=5 ))*100 as negativereachout,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=4 and t2.status_id=2 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=4 ))*100 as willdolater_reachout,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=6 and t2.status_id=5 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=6 ))*100 as positive_negative,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=6 and t2.status_id=4 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=6 ))*100 as positive_willdolater FROM user_details where user_details.status='active' and user_details.user_id = (" + str(
                request.user.id) + ")"
        else:
            query = "SELECT user_details.*,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=1 and t2.status_id=8 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=1 ))*100 as open_to_open_rpem,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=8 and t2.status_id=2 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=8 ))*100 as open_rpem_to_reachout,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=2 and t2.status_id=3 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=2 ))*100 as reachout_to_tentative,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=3 and t2.status_id=6 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=3 ))*100 as tentative_to_verypositive,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=6 and t2.status_id=9 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=6 ))*100 as positive_to_verypositive,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=9 and t2.status_id=7 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=9 ))*100 as verypositive_to_closure,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=8 and t2.status_id=4 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=8 ))*100 as open_to_willdolater,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=8 and t2.status_id=5 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=8 ))*100 as open_to_negative,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=2 and t2.status_id=4 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=2 ))*100 as reachout_to_willdolater,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=2 and t2.status_id=5 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=2 ))*100 as rechout_negative,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=3 and t2.status_id=4 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=3 ))*100 as tentative_willdolater,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=3 and t2.status_id=5 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=3 ))*100 as tentative_negative,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=5 and t2.status_id=2 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=5 ))*100 as negativereachout,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=4 and t2.status_id=2 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=4 ))*100 as willdolater_reachout,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=6 and t2.status_id=5 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=6 ))*100 as positive_negative,((SELECT count(*) from tblcallevents t1 join tblcallevents t2 on t1.nextCFID=t2.id where (t1.status_id=6 and t2.status_id=4 ) and t1.assignedto_id=user_details.user_id  )/(SELECT count(distinct t1.cid_id) from tblcallevents t1  where t1.assignedto_id=user_details.user_id  and t1.status_id=6 ))*100 as positive_willdolater FROM user_details where user_details.status='active' and user_details.user_id in %s" % (
                tuple(usrs),)
    else:
        if user.type_id == 3:
...
def school_visit_reports(request):
    ...
    if request.method == "POST":
        ...
        if user.type_id == 3:
            ...
            tentativetopositivequery = " WHERE user_details.user_id = " + str(
                request.user.id) + " and company_master.partnerType_id='11' and (cr_event.appointmentdatetime BETWEEN '" + from_date + "' and '" + to_date + "') group by cr_event.id"
        else:
...
def meeting_proposal(request):
    user = models.users.objects.get(user=request.user)
    # status = models.status.objects.filter()
    sql = "SELECT cr_event.*,cr_status.name as current_status ,cr_action.name as current_action_type,user_details.name,last_event.appointmentdatetime as last_task_date,last_action.name as last_task_activity,last_event.remarks as last_remark,lpurpose.name as last_purpose,npurpose.name as next_purpose,lstatus.name as last_status,last_event.actontaken as last_action_taken,next_event.appointmentdatetime as next_task_date,nx_action.name as next_task_activity,next_event.remarks as next_remarks,company_master.*,init_call.draft as init_draft,partner_master.name as partner_type,init_call.*,company_contact_master.* from tblcallevents cr_event left join tblcallevents last_event on cr_event.lastCFID=last_event.id left join action cr_action on cr_action.id=cr_event.actiontype_id left join status cr_status on cr_status.id=(select ff.status_id from tblcallevents ff where ff.id=(select max(id) from tblcallevents where cid_id=cr_event.cid_id) ) left join tblcallevents next_event on cr_event.nextCFID=next_event.id  left join init_call on init_call.id=cr_event.cid_id left join company_master on company_master.id=init_call.cmpid_id left join action last_action on last_action.id=last_event.actiontype_id left join purpose lpurpose on lpurpose.id=last_event.purpose_id left join purpose npurpose on npurpose.id=next_event.purpose_id left join status lstatus on lstatus.id=last_event.status_id left join action nx_action on nx_action.id=next_event.actiontype_id left join company_contact_master on company_master.id=company_contact_master.company_id left join partner_master on partner_master.id=company_master.partnerType_id join user_details on cr_event.assignedto_id=user_details.user_id"
...
def meeting_dashboard(request):
    user = models.users.objects.get(user=request.user)
    sql = "SELECT cr_event.*,cr_status.name as current_status,cr_action.name as current_action_type,user_details.name,last_event.appointmentdatetime as last_task_date,last_action.name as last_task_activity,last_event.remarks as last_remark,lpurpose.name as last_purpose,npurpose.name as next_purpose,lstatus.name as last_status,last_event.actontaken as last_action_taken,next_event.appointmentdatetime as next_task_date,nx_action.name as next_task_activity,next_event.remarks as next_remarks,(select t5.remarks from tblcallevents t5 where t5.id=(select MAX(id) from tblcallevents where cid_id=cr_event.cid_id) ) as current_remark,company_master.*,partner_master.name as partner_type,init_call.*,company_contact_master.* from tblcallevents cr_event left join tblcallevents last_event on cr_event.lastCFID=last_event.id left join action cr_action on cr_action.id=cr_event.actiontype_id left join status cr_status on cr_status.id=(select ff.status_id from tblcallevents ff where ff.id=(select max(id) from tblcallevents where cid_id=cr_event.cid_id) ) left join tblcallevents next_event on cr_event.nextCFID=next_event.id  left join init_call on init_call.id=cr_event.cid_id left join company_master on company_master.id=init_call.cmpid_id left join action last_action on last_action.id=last_event.actiontype_id left join purpose lpurpose on lpurpose.id=last_event.purpose_id left join purpose npurpose on npurpose.id=next_event.purpose_id left join status lstatus on lstatus.id=last_event.status_id left join action nx_action on nx_action.id=next_event.actiontype_id left join company_contact_master on company_master.id=company_contact_master.company_id left join partner_master on partner_master.id=company_master.partnerType_id join user_details on cr_event.user_id=user_details.user_id"
    common_condition=" and ((last_event.actontaken='yes' and last_event.status_id='2' and cr_event.status_id='3'))  group by init_call.cmpid_id "
    if request.method=="POST":
        if user.type_id == 1:
            users = models.users.objects.filter()
        elif user.type_id == 2:
        ...
        if request.POST['from'] != '' and request.POST['to'] != '':
            if request.POST['user'] != '':
                data = models.tblcalls.objects.raw(sql+
                    ' WHERE cr_event.user_id =' +
                    request.POST[
                        "user"] + ' and cr_event.fwd_date between "' + from_date + '" and "' + to_date + '" '+common_condition+'  order by cr_event.id desc'
                )
            else:
                if user.type_id == 3:
                    data = models.tblcalls.objects.raw(sql+
                        ' WHERE cr_event.user_id =' + str(
                            request.user.id) + ' and cr_event.fwd_date between "' + from_date + '" and "' + to_date + '" '+common_condition+'    order by cr_event.id desc')
                else:
                    ...
                    data = models.tblcalls.objects.raw(sql+
                        ' WHERE cr_event.user_id in %s' % (
                            tuple(
                                us),) + ' and cr_event.fwd_date between "' + from_date + '" and "' + to_date + '" '+common_condition+'   order by cr_event.id desc')
