...
<div class="wrapper">
  ...
  <form class="p-3" method="POST" action="<?=base_url();?>Menu/ReportVisitIMP">
  <input type="date" name="date" class="mr-2">
  <select name="imp" class="mr-2">
      <option value='1'>All</option>
      <?php $imp=$this->Menu_model->get_user_imp();foreach($imp as $im){?>
      <option value='<?=$im->imid?>'><?=$im->fullname?></option>
      <?php } ?>
  </select>
  ...
  <button type="submit" class="bg-primary text-light">Filter</button>
    ...
    <?php if(isset($_POST["date"])){$date = $_POST["date"]; $imp = $_POST["imp"];?>
    <section class="content">
     <div class="container-fluid">
        <div class="col-sm col-md-12 col-lg-12 m-auto">
            <div class="card card-primary card-outline p-3">
                <center><h4>Task Report Summary of IMPs</h4></center><hr>
                <?php date_default_timezone_set("Asia/Kolkata");
                    $aai=1; $date = date('Y-m-d', strtotime('+0 day'));
                    $vtask1=$this->Menu_model->get_livevisitimp1($imp,$date);?>
                    <center><h4><b>Total Visit Plan : <?=$vtask1[0]->tvplan?> | Total Visit Done : <?=$vtask1[0]->vdone?> | Total Visit Pending : <?=$vtask1[0]->vpen?></b></h4></center>
                    <div class="row">
                    <?php $vtask2=$this->Menu_model->get_livevisitimp2($imp,$date);
                    foreach($vtask2 as $vt){?>
                
                        <div class="col-lg-2 col-md-3 col-sm-12 bg-danger card p-3 border border-white text-center"><b>
                           <?=$vt->taskname?> : <?=$vt->tvplan?></b>
                        </div>
     ...
     <div class="col-sm col-md-12 col-lg-12 m-auto">
         <div class="card card-primary card-outline">
              <div class="card-body box-profile row">
                                        ...
                                        $nxtdtask=$this->Menu_model->get_reportvisitimp($imp,$date);
                                    ...
                                    <?php $repa = $this->Menu_model->get_replacereqbytid($tid);
                                        foreach($repa as $rep){?>
                                        <div class="col-2 bg-success card p-3 border border-white">
                                            <b><?=$rep->type?></b><hr>
                                            <b><?=$rep->model_name?></b><hr>
                                            <b><?=$rep->part_name?></b><hr>
                                            <b><?=$rep->remark?></b><hr>
                                            <video class="embed-responsive-item img-thumbnail" src="https://stemoppapp.in/<?=$rep->modelimg?>" height="300" muted controls></video></a>
