...
radioButtons.forEach(function(radio) {
    radio.addEventListener('change', function() {
       ...
       // End school_targte_date_card

// Start timeline_settings_card
    if (selectedFilter === 'Time Line Setting') {
        $('#timeline_settings_card').fadeIn();
        $('#defaultTaskCard, #task_action_card').hide();

        $('#timeline_settings_lists').change(function () {
        var timeline_settings = $(this).val(); // Get selected value
        var timeline_settings_text = $(this).find(":selected").text().trim(); // Get the selected text

        $('#taskplanningimg').hide();
        $('#taskplanning_loader').fadeIn();
        $('#updatetasklists').empty(); // Clear previous tasks

        $("#taskTypeListByDepartmentID").val(timeline_settings).prop('disabled', false).css('pointer-events', 'none');
        $('#taskActionListByDepartmentID').empty();

        // Fetch Task Action List
        $.ajax({
            url: '<?=base_url();?>Menu/GetTaskActionListUsingTaskTypeName',
            type: "POST",
            data: { task_type_name: timeline_settings },
            cache: false,
            success: function (result) {
                $('#taskActionListByDepartmentID').html(result).prop('disabled', false).css('pointer-events', 'none');
                $('#taskActionListByDepartmentID option').each(function () {
                        if ($(this).text().trim() === timeline_settings_text) {
        ...
        $.ajax({
            url: '<?=base_url();?>Menu/GetNewTimelineSettings',
            type: "POST",
            data: { timeline_settings_text: timeline_settings_text },
            cache: false,
            success: function (result) {
                $('#taskplanning_loader').hide();
                if (result.trim() !== '') {
                    $('#maintaskcard, #task_action_card').fadeIn();
                    $('#updatetasklists').html(result);

                    var optionCount = $('#updatetasklists option').length;
                    $("#updatetasklists_text").text('Total Task: ' + optionCount).css('color', 'green');
                }
            }
        });
    ...
    $('#timeline_settings_card').hide();
    $("#taskTypeListByDepartmentID").prop('disabled', false).css('pointer-events', 'auto');
    $('#taskActionListByDepartmentID').prop('disabled', false).css('pointer-events', 'auto');
}

// End timeline_settings_card
