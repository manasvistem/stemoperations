    ...
    <section class="content">
      <div class="container-fluid">
        <div class="row">
          <div class="col-12">
              <div>
                  <form class="p-3" method="POST" action="UserTaskDetail">
                      <input type="date" name="sdate" class="mr-2" value="<?=$sd; ?>">
                      <input type="date" name="edate" class="mr-2" value="<?=$ed; ?>">
                      <select name="departmet" class="mr-2">
                          <?php $dept = $this->Menu_model->get_depatment();
                                foreach($dept as $dep){ ?>
                        ...
                        <div class="table-responsive">
                            <div class="table-responsive">
                                <div class="pdf-viwer">
                                    ...
                                    <tbody>
                                        <?php $userd = $this->Menu_model->get_usertaskdetail($sd,$ed,$departmet);
                                        foreach($userd as $userd){
                                            $fullname = $userd->fullname;
                                            $piid = $this->Menu_model->get_user_byfname($fullname);
                                            $piid = $piid[0]->id;
                                            
                                            $ureportd = $this->Menu_model->get_userreportdetail($sd,$ed,$fullname); ?>
                                    <tr>
                                        <td><?=$userd->fullname?></td>
                                        <td><a href="taskdetaildw/<?=$piid?>/<?=$sd?>/<?=$ed?>/Call"><?=$userd->tcall?></a></td>
                                        <td><a href="taskdetaildw/<?=$piid?>/<?=$sd?>/<?=$ed?>/Visit"><?=$userd->visit?></td>
                                        <td><a href="taskdetaildw/<?=$piid?>/<?=$sd?>/<?=$ed?>/Utilisation"><?=$userd->utilisation?></td>
                                        <td><a href="taskdetaildw/<?=$piid?>/<?=$sd?>/<?=$ed?>/Communication"><?=$userd->outbond?></td>
                                        <td><a href="taskdetaildw/<?=$piid?>/<?=$sd?>/<?=$ed?>/Report"><?=$userd->report?></td>
