  ...
  <div class="content-wrapper">
    ...
    <div class="card">
              ...
              <div class="card-body">
                ...
                <?php $i=1;
                foreach($BDRPRT as $BPRT){?>
                <div class="card col-sm col-md-12 col-lg-12 m-auto p-3 border rounded">
                    Request Type : <?=$rtype=$BPRT->request_type;?> | 
                    Total Request : <?=$BPRT->task;?>
                </div>
                    <?php 
                    $BDRP=$this->Menu_model->get_BDRP($rtype);
                    foreach($BDRP as $dt){$cdate=date('Y-m-d H:i:s');$tid=$dt->id;?>
                      <div class="card col-sm col-md-3 col-lg-3 m-auto p-3 bg-danger border rounded">
                          <a href="BDRSDetail/<?=$tid?>">
                          Request Type<br><b><?=$dt->request_type?></b><hr>
                          Request Date<br><b><?=$dt->sdatet?></b><hr>
                          BD Name<br><b><?=$dt->bd_name?></b><hr>
                          Remark<br><b><?=$dt->remark?><?=$dt->cpmo?><?=$dt->visitdate?><?=$dt->vlocation?><?=$dt->expectation?></b><hr>
                          Company Name<br><b><?=$dt->cname?><?=$dt->cpname?></b><hr>
                          Target Date<br><b><?=$tdt=$dt->targetd?></b><hr></a>
                          <b><?=$this->Menu_model->timediff($tdt,$cdate)?></b><hr>
                          <button type="button" class="btn btn-light mt-1" id="add_comment<?=$i?>" value="<?=$tid?>">Comment</button>
                          <button type="button" class="btn btn-light mt-1" id="assign_to<?=$i?>" value="<?=$tid?>">Create Stage</button>
                      </div>
                    <?php $i++;}}?>
                </div>
        </div>
