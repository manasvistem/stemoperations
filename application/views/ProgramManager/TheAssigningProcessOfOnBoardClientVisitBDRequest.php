          ...
          <div class="card">
            ...
            <div class="card p-3" style="background:#dcf3ffed">
              ...
              <div class="mb-4" id="inauguration_Card">
                ...
                <hr>
                <?=form_open('Menu/BDRequestAssignToProcessOnBoardClientSchoolVisit')?>
                <div class="row">
                  <div class="col-md-12">
                ...
                <div class="row">
                    <div class="col-md-6 card flexwrapcontent1">
                        ...
                        <select id="schoolloop_<?=$k;?>" name="sid[]" class="form-control" required >
                          <option value="">Select School</option>
                            <?php foreach($getSPDData as $getSPDDatas){?>
                          <option value="<?=$getSPDDatas->id?>"><?=$getSPDDatas->sname?></option>
                          <?php } ?>
                        </select>
                        </div>
                    </div>
...
<script>
  $(document).ready(function () {
    $('select[name="sid[]"]').change(function() {
        ...
        var number = selectedId.match(/\d+/); // Extract the number using regex
        var AssignToidis = '#AssignTo_'+number[0];
              ...
              $.ajax({
                ...
                success: function b(result) {
                    if(result !== ''){
                        // console.log(AssignToidis);
                        $(AssignToidis).val(result).change(); // Set selected value to result
                    }else{
                        alert("No PIA in This SPD");
                    }
                 
                //   $("#AssignToidis").val(result);
                }
              });
            


        // console.log("Dropdown ID IS:", number[0]);
        // console.log("AssignTo ID IS:", AssignToidis);
        // console.log("Dropdown ID:", selectedId);
