  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Installation Review</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
    ...
    <form action="<?=base_url();?>Menu/addurwag" method="post" enctype="multipart/form-data">
          ...
          <input type="hidden" name="urtid" value="<?=$urtid?>">
      ...
      <div class="container-fluid">
        ...
        <div class="card-body p-2">
          ...
          <div class="col-md-9">
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Review Installation</h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
                   <div class="card-body box-profile">
                    <div class="row">
                        <b>School Readiness Check List</b>
                        <ul>
                            <?php $task = $this->Menu_model->get_tidbypage($sid,'page1'); 
                            foreach($task as $td){
                            $stid = $td->tid;
                            $queans = $this->Menu_model->get_ans($sid,$stid);
                        ...
                        <b>Pre - Intervention Enquiry for Installation Check List</b>
                        <ul>
                            <?php $task = $this->Menu_model->get_tidbypage($sid,'page2'); 
                            foreach($task as $td){
                            $stid = $td->tid;
                            $queans = $this->Menu_model->get_ans($sid,$stid);
                            foreach($queans as $qa){?>
                        ...
                        <b>During installation Check-List From Check List</b>
                        <ul>
                            <?php $task = $this->Menu_model->get_tidbypage($sid,'page6'); 
                            foreach($task as $td){
                            $stid = $td->tid;
                            $queans = $this->Menu_model->get_ans($sid,$stid);
                            foreach($queans as $qa){?>
                    ...
                    <b>Data</b> 
                    <div class="row">
                    <?php $spdtask = $this->Menu_model->get_tidbyspd('2780');
                    foreach($spdtask as $spdt){
                    $spt = $spdt->tid;
                    $vupdate = $this->Menu_model->get_visitstupdate('2780',$spt);
                    foreach($vupdate as $vu){
                    ?>
                    <div class="col-sm col-lg-4">
                       <a href="<?=base_url();?><?=$vu->ans1?>" target="_blank">
                          <img src="<?=base_url();?><?=$vu->ans1?>"  class="img-thumbnail" width="100%">
                       </a>
                    </div>
                    <?php }} ?>
                    </div>
                    ...
                    <div class="col-lg-12">
                    <?php $type='Upload Installation Report'; $report= $this->Menu_model->get_reportsidntype($sid,$type);?>
                    <object data="<?=base_url();?><?=$report[0]->filepath?>" type="application/pdf" width="100%" height="800px"></object>
                      </div>      
                    </div>
                        <div class="from-group mt-3">
                            <h5 class="text-center"><input type="hidden" name="que[]" value="">Rating Installation</h5>
                            <div class="rating">
                   ...
                   <textarea id="remark" name="remark" class="form-control  p-3  mt-2" placeholder="Remark" required></textarea>
                   <!-- /.card-body -->
