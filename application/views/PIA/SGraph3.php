    ...
    <section class="content">
      <div class="container-fluid">
          <div class="card col-12  p-3 text-center">
              <center><h5>Client Wise School Graph</h5></center><hr>
...
<script type="text/javascript">
    ...
    function drawChart12() {
        ...
        <?php
        $clientData = $this->Menu_model->get_SPDbypiclientw($uid);
        foreach ($clientData as $client) {
            $clientName = $client->clientname . " (" . $client->cont . ")";
            echo "data.addRow(['$clientName', $client->cont, '$client->clid', '$uid']);";
        }
        ?>
        ...
        google.visualization.events.addListener(chart, 'select', function() {
            ...
            if (selection) {
                var clid = data.getValue(selection.row, 2);
                var uuid = data.getValue(selection.row, 3);
                var code = 1;
                // Redirect to another URL with clid and uuid as parameters
                window.location.href = '<?= base_url();?>Menu/SGraph3/' + clid + '/' + code;
            }
        });
          ...
          <div class="card col-12  p-3 text-center">
              ...
              <div class="container-fluid card p-5" id="data-container">
                <div class="row text-center" id="grid-view">
                    ...
                    <?php 
                         $mdata = $this->Menu_model->get_schoolbycode2($uid,$clid,$code);
                         foreach($mdata as $dt){
                ...
                <div id="tabular-view" style="display: none;">
                    <div class="card p-3 col-lg-4 col-sm m-auto bg-light">
                        <?php
                        $cityData = $this->Menu_model->get_SPDbypiclientw($uid);
                        foreach ($clientData as $client) {?>
                            <b><a href="<?= base_url(); ?>Menu/SGraph3/<?= $client->clid ?>/<?= $code ?>"><?= $client->clientname ?> - <?= $client->cont ?></a></b><hr>
                        <?php } ?>
                        <b><a href="<?= base_url(); ?>Menu/SGraph3/<?= $code ?>">View All</a></b>
                    </div>
                </div>
