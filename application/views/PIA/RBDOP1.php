    ...
    <section class="content">
      <div class="container-fluid">
          <div class="card col-12  p-3 text-center">
              <center><h5>Open BD Request BD Name Wise</h5></center><hr>
...
<?php
  ...
  <script type="text/javascript">
        ...
        function drawChart12() {
          ...
          <?php
          $code='10';
          $status = $this->Menu_model->get_BDRbyPMG2();
          foreach ($status as $st) {
            $statusName = $st->bdname . " (" . $st->cont . ")";
            echo "data.addRow(['$statusName', $st->cont, '$st->bdid', '$uid']);";
          }
          ?>
          ...
          google.visualization.events.addListener(chart, 'select', function() {
            ...
            if (selection) {
              var bdid = data.getValue(selection.row, 2);
              var uuid = data.getValue(selection.row, 3);
              var code = 10;
              window.location.href = '<?=base_url();?>Menu/RBDOP1/' + bdid + '/' + code;
            }
          });
          ...
          <div class="card col-12  p-3 text-center">
              ...
              <div class="container-fluid card p-5" id="data-container">
                <div class="row text-center" id="grid-view">
                     <?php $BDRequest = $this->Menu_model->get_BDRequestbybdid($bd_id,$code);
                ...
                <div id="tabular-view" style="display: none;">
                    ...
                    <?php
                      $status = $this->Menu_model->get_BDRbyPMG1();
                      foreach ($status as $st) {?>
                      <b><a href="<?=base_url();?>Menu/RBDOP/<?=$st->rysn?>/<?=$code?>"><?=$st->rtype?> - <?=$st->cont?></a></b><hr>
                      <?php } ?>
                      <b><a href="<?=base_url();?>Menu/RBDOP/<?=$code?>">View All</a></b>
                      </div>
                </div>  
