    ...
    <section class="content">
      <div class="container-fluid">
          <div class="card col-12  p-3 text-center">
              <center><h5>Month wise day and type Holidays</h5></center><hr>
        ...
        var stackedData = {
            ...
            datasets: [{
                    ...
                    data: [
                        ...
                        for ($month = 4; $month <= 12; $month++) {
                        $monthName = DateTime::createFromFormat('!m', $month)->format('F');
                        $mdata = $this->Menu_model->get_pig4($uid,$month,$financialYear);
                        foreach($mdata as $dt){?>
                        <?=$dt->days?>,
                ...
                <?php $i=1; $htype = $this->Menu_model->get_holitype($uid);
                      foreach($htype as $ht){$htp = $ht->type;?>
                
                {
                    ...
                    data: [
                        ...
                        for ($month = 4; $month <= 12; $month++) {
                        $monthName = DateTime::createFromFormat('!m', $month)->format('F');
                        $mdata = $this->Menu_model->get_pig4c1($uid,$month,$financialYear,$htp);
                        foreach($mdata as $dt){?>
                        <?=$dt->days?>,
                        <?php }} ?> 
                        ],
