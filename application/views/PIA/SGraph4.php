    ...
    <section class="content">
      <div class="container-fluid">
          <div class="card col-12  p-3 text-center">
              <center><h5>State Wise School Graph</h5></center><hr>
...
<script type="text/javascript">
  ...
  function drawChart12() {
    ...
    <?php
    // Fetch data using your PHP function
    $state = $this->Menu_model->get_SPDbypistatew($uid);
    foreach ($state as $st) {
      $statusName = $st->statename . " (" . $st->cont . ")";
    ...
    google.visualization.events.addListener(chart, 'select', function() {
      ...
      if (selection) {
        ...
        window.location.href = '<?= base_url(); ?>Menu/SGraph4/' + stid + '/' + code;
      }
    });
          ...
          <div class="card col-12  p-3 text-center">
              ...
              <div class="container-fluid card p-5" id="data-container">
                <div class="row text-center" id="grid-view">
                    ...
                    <?php 
                         $mdata = $this->Menu_model->get_schoolbycode3($uid,$stid,$code);
                         foreach($mdata as $dt){
                ...
                <div id="tabular-view" style="display: none;">
                    ...
                    <?php
                      $status = $this->Menu_model->get_SPDbypistatew($uid);
                      foreach ($status as $st) {?>
                      <b><a href="<?=base_url();?>Menu/SGraph4/<?=$st->stid?>/<?=$code?>"><?=$st->statename?> - <?=$st->cont?></a></b><hr>
                      <?php } ?>
                      <b><a href="<?=base_url();?>Menu/SGraph4/<?=$code?>">View All</a></b>
                      </div>
                </div>  
