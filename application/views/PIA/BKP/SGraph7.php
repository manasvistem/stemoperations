    ...
    <section class="content">
      <div class="container-fluid">
          <div class="card col-12  p-3 text-center">
              <center><h5>Teacher Wise School Graph</h5></center><hr>
...
<script type="text/javascript">
  ...
  function drawChart12() {
    ...
    data.addColumn('string', 'Total Teachers');
    data.addColumn('number', 'School Count');
    data.addColumn('string', 'TotalTeachers');
    data.addColumn('string', 'uid');

    // PHP data generation logic here
    <?php
    $teacher_data = $this->Menu_model->get_SPDbypisteacherw($uid);
    foreach ($teacher_data as $st) {
      $total_teachers = $st->total_teachers;
      $school_count = $st->sname_count;
      echo "data.addRow(['$total_teachers', $school_count, '$total_teachers', '$uid']);";
    }
    ?>
    ...
    google.visualization.events.addListener(chart, 'select', function () {
      ...
      if (selection) {
        var totalTeachers = data.getValue(selection.row, 2);
        var uuid = data.getValue(selection.row, 3);
        var code = 1;
        // Redirect to another URL with totalTeachers and uuid as parameters
        window.location.href = '<?= base_url(); ?>Menu/SGraph7/' + totalTeachers + '/' + code;
      }
    });
          ...
          <div class="card col-12  p-3 text-center">
              ...
              <div class="container-fluid card p-5" id="data-container">
                <div class="row text-center" id="grid-view">
                    ...
                    <?php 
                         $mdata = $this->Menu_model->get_schoolbycode6($uid,$total_teachers,$code);
                         foreach($mdata as $dt){
                    ...
                    ?>
                        ...
                        <div class="col-sm-12 col-md-4 col-lg-4 mb-4 filter-item">
                          <div class="card p-3 border rounded border-success hover-div d-flex flex-column align-items-stretch h-100 text-dark">
                                 ...
                                 Total Teacher<br><b><?=$dt->total_teachers?></b><hr>
                ...
                <div id="tabular-view" style="display: none;">
                    ...
                    <?php
                      $status = $this->Menu_model->get_SPDbypisteacherw($uid);
                      foreach ($status as $st) {?>
                      <b><a href="<?=base_url();?>Menu/SGraph7/<?=$st->total_teachers?>/<?=$code?>"><?=$st->total_teachers?> - <?=$st->sname_count?></a></b><hr>
                      <?php } ?>
                      <b><a href="<?=base_url();?>Menu/SGraph7/<?=$code?>">View All</a></b>
                      </div>
                </div>  
