    ...
    <section class="content">
      <div class="container-fluid">
          <div class="card col-12  p-3 text-center">
              <center><h5>City Wise School Graph</h5></center><hr>
...
<div id="piechart3d3" style="width: 100%; height: 500px;"></div>
  ...
  <script type="text/javascript">
        ...
        function drawChart12() {
          ...
          <?php
          $cityData = $this->Menu_model->get_SPDbypicityw($uid);
          foreach ($cityData as $city) {
            $cityName = $city->cityname . " (" . $city->cont . ")";
            echo "data.addRow(['$cityName', $city->cont, '$city->ctid', '$uid']);";
          }
          ?>
          ...
          google.visualization.events.addListener(chart, 'select', function() {
            ...
            if (selection) {
              var ctid = data.getValue(selection.row, 2);
              var uuid = data.getValue(selection.row, 3);
              var code = 1;
              // Redirect to another URL with ctid and uuid as parameters
              window.location.href = '<?=base_url();?>Menu/SGraph2/' + ctid + '/' + code;
            }
          });
          ...
          <div class="card col-12  p-3 text-center">
              ...
              <div class="container-fluid card p-5" id="data-container">
                <div class="row text-center" id="grid-view">
                    ...
                    <?php 
                         $mdata = $this->Menu_model->get_schoolbycode1($uid,$stid,$code);
                         foreach($mdata as $dt){
                ...
                <div id="tabular-view" style="display: none;">
                    <div class="card p-3 col-lg-4 col-sm m-auto bg-light">
                        ...
                        $cityData = $this->Menu_model->get_SPDbypicityw($uid);
                        foreach ($cityData as $city) {?>
                            <b><a href="<?= base_url(); ?>Menu/SGraph2/<?= $city->ctid ?>/<?= $code ?>"><?= $city->cityname ?> - <?= $city->cont ?></a></b><hr>
                        <?php } ?>
                        <b><a href="<?= base_url(); ?>Menu/SGraph2/<?= $code ?>">View All</a></b>
                    </div>
                </div>
