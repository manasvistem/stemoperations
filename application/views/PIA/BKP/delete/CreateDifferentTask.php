       ...
       <div class="row p-3">
           <div class="col-sm col-md-6 col-lg-6 m-auto">
              ...
              <div class="card-body box-profile">
                  <form action="<?=base_url();?>Menu/createaot" method="post">
                  <input type="hidden" name="uid" value="<?=$uid?>">
                  <div class="form-group">
                    <label>Task Type</label>
                    <select name="tasktype" class="form-control">
                    <option>Client Engagement Activity</option>
                    <option>Virtual Program</option>
                    <option>Report to govt. Officers</option>
                    <option>Total client visit</option>
                    <option>Total Demo</option>
                    </select>
                  </div>
                  
                  <div class="form-group">
                    <label>Action</label>
                    <select type="text" name="action" class="form-control">
                        <option>from Office</option>
                        <option>from Field</option>
                    </select>
                  </div>
                  
                  <div class="form-group">
                    <label>Link with Project</label>
                    <input list="pcode" name="pcode" class="form-control">
                    <datalist id="pcode">
                        <option value="">Select Project</option>
                        <?php foreach($project as $pro){?>
                        <option><?=$pro->project_code?></option>
                        <?php } ?>
                    </datalist>
                  </div>
                  
                  <div class="form-group">
                    <label>Link with School</label>
                    <input list="sid" name="sid" class="form-control">
                    <datalist id="sid">
                        <option value="">Select Project</option>
                  ...
                  <div class="form-group">
                    <label>Link with BD Request</label>
                    <input list="brid" name="brid" class="form-control">
                    <datalist id="brid">
                        <option value="">Select Project</option>
                        <?php foreach($bdrequest as $bdr){?>
                        <option value="<?=$sp->id?>"><?=$bdr->request_type?>,<?=$bdr->cname?>,<?=$bdr->remark?>,<?=$bdr->cpname?></option>
                        <?php } ?>
                    </datalist>
                  </div>
                  <div class="form-group">
                    <label>Remark</label>
                    <textarea class="form-control" name="taskremark" placeholder="Remark" ></textarea>
                  </div>
