    ...
    <section class="content">
      <div class="container-fluid">
          <div class="card col-12  p-3 text-center">
              <center><h5>Students Wise School Graph</h5></center><hr>
  ...
  <script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
            <script type="text/javascript">
              ...
              function drawChart12() {
                ...
                data.addColumn('string', 'Total Students');
                data.addColumn('number', 'School Count');
                data.addColumn('string', 'TotalStudents');
                data.addColumn('string', 'uid');
            
                // PHP data generation logic here
                <?php
                $student_data = $this->Menu_model->get_SPDbypisstudentw($uid);
                foreach ($student_data as $st) {
                  $total_students = $st->total_students;
                  $school_count = $st->sname_count;
                  echo "data.addRow(['$total_students', $school_count, '$total_students', '$uid']);";
                }
                ?>
                ...
                google.visualization.events.addListener(chart, 'select', function () {
                  ...
                  if (selection) {
                    var totalStudents = data.getValue(selection.row, 2);
                    var uuid = data.getValue(selection.row, 3);
                    var code = 1;
                    // Redirect to another URL with totalStudents and uuid as parameters
                    window.location.href = '<?= base_url(); ?>Menu/SGraph8/' + totalStudents + '/' + code;
                  }
                });
          ...
          <div class="card col-12  p-3 text-center">
              ...
              <div class="container-fluid card p-5" id="data-container">
                <div class="row text-center" id="grid-view">
                   ...
                   <?php 
                            $mdata = $this->Menu_model->get_schoolbycode7($uid, $total_students, $code);
                            ...
                            <div class="col-sm-12 col-md-4 col-lg-4 mb-4 filter-item">
                                <div class="card p-3 border rounded border-success hover-div d-flex flex-column align-items-stretch h-100 text-dark">
                                    <div>
                                        ...
                                        Total Student<br><b><?= $dt->total_students ?></b><hr>
                ...
                <div id="tabular-view">
                    <div class="card p-3 col-lg-4 col-sm m-auto bg-light">
                        ...
                        $student_data = $this->Menu_model->get_SPDbypisstudentw($uid);
                        foreach ($student_data as $st) {?>
                            <b><a href="<?= base_url(); ?>Menu/SGraph8/<?= $st->total_students ?>/<?= $code ?>"><?= $st->total_students ?> - <?= $st->sname_count ?></a></b><hr>
                        <?php } ?>
                        <b><a href="<?= base_url(); ?>Menu/SGraph8/<?= $code ?>">View All</a></b>
                    </div>
                </div>
