...
<div class="container-xxl flex-grow-1 container-p-y">
  <div class="card p-3">
    <h3 class="card-header text-center">School Timeline Data</h3>
    <hr>
    <?php 
$uniqueAcademicYears = array_unique(array_column($schooltimelineDatas, 'academicyear'));
?>
    <div class="row">
      <div class="col-md-9"></div>
      <div class="col-md-3">
      <form action="<?=base_url();?>Menu/CheckSchoolTimelineData" method="post" style="align-items: center; display: flex ; padding: 10px;">
       <div class="m-2">
          <label for="exampleFormControlSelect1" class="form-label">Academic Year</label>
          <select class="form-select" name="academic_year" id="exampleFormControlSelect1" aria-label="Default select example">
            <?php foreach($uniqueAcademicYears as $uniqueAcademicYear){ ?>
              <option value="<?=$uniqueAcademicYear;?>"><?=$uniqueAcademicYear;?></option>
            <?php } ?>
          </select>
        </div>
    ...
    <div class="table-responsive text-nowrap">
      <?php
        $schooltimelineDatascnt =  sizeof($schooltimelineDatas);
        if($schooltimelineDatascnt > 0){
        ?>
      <table id="example1" class="table table-striped">
        ...
        <tbody>
          <?php 
            $i = 1; 
            foreach($schooltimelineDatas as $pdata) {
            ?>
          <tr>
            <td><?= $i ?></td>
            <td><?= $pdata->academicyear ?></td>
            <td>
              <a href="<?=base_url().'Menu/ViewSchoolTimelineData/'.$pdata->id.'/'.$pdata->sid.'/'.$pdata->ptimeline_id ?>">
                  <?= $pdata->sname ?>
              </a>
            </td>
