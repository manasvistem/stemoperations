...
<form action="<?php echo base_url();?>/Menu1/updateTask" name="welcome_message" method="post">
        <div class="card shadow p-4">
            <h3 class="text-center mb-3">Welcome Message</h3>
    ...
    <script>
        function sendWhatsApp(clientcontact) {
            let phone = clientcontact; // Replace with actual number
            let message = encodeURIComponent("Welcome on board");
            window.open(`https://wa.me/${phone}?text=${message}`, "_blank");
        }

        function sendEmail() {
            let email = ""; // Replace with actual email
            let subject = encodeURIComponent("Inquiry");
            let body = encodeURIComponent("Hello, I need some information.");
            window.location.href = `mailto:${email}?subject=${subject}&body=${body}`;
        }

        function toggleRemarks() {
            let remarkSection = document.getElementById("remarkSection");
            remarkSection.classList.toggle("d-none");
        }
    </script>
