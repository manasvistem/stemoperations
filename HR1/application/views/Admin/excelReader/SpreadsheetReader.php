 ...
 */
	class SpreadsheetReaderA implements SeekableIterator, Countable
	{
		...
		/**
		 * @var SpreadsheetReaderA_* Handle for the reader object
		 */
		...
		public function __construct($Filepath, $OriginalFilename = false, $MimeType = false)
		{
			...
			{
				throw new Exception('SpreadsheetReaderA: File ('.$Filepath.') not readable');
			}
			...
			{
				throw new Exception('SpreadsheetReaderA: Original file (2nd parameter) path is not a string or a scalar value.');
			}
			if (!empty($MimeType) && !is_scalar($MimeType))
			{
				throw new Exception('SpreadsheetReaderA: Mime type (3nd parameter) path is not a string or a scalar value.');
			}
			...
			if ($this -> Type == self::TYPE_XLS)
			{
				...
				$this -> Handle = new SpreadsheetReaderA_XLS($Filepath);
			...
			switch ($this -> Type)
			{
				case self::TYPE_XLSX:
					...
					$this -> Handle = new SpreadsheetReaderA_XLSX($Filepath);
					break;
				case self::TYPE_CSV:
					self::Load(self::TYPE_CSV);
					$this -> Handle = new SpreadsheetReaderA_CSV($Filepath, $this -> Options);
					break;
				case self::TYPE_XLS:
				...
				case self::TYPE_ODS:
					...
					$this -> Handle = new SpreadsheetReaderA_ODS($Filepath, $this -> Options);
					break;
			}
		}
		...
		private static function Load($Type)
		{
			...
			{
				throw new Exception('SpreadsheetReaderA: Invalid type ('.$Type.')');
			}
			...
			if (!class_exists('SpreadsheetReaderA_'.$Type, false))
			{
				require(dirname(__FILE__).DIRECTORY_SEPARATOR.'SpreadsheetReaderA_'.$Type.'.php');
			}
		}
		...
		public function seek($Position)
		{
			if (!$this -> Handle)
			{
				throw new OutOfBoundsException('SpreadsheetReaderA: No file opened');
			}
